...
var airdropChance = 150;
var megaJackpotChance = 300;
var previousTRXdist = new BigNumber(0);

var INSTANTWIN_NEEDED = 300;
var SAFETYDEPOSITWIN_NEEDED = 150;
...
function checkNetwork(_callback) {
  ...
  async function setupContracts() {
      ...
      try{
        TronHeistContract = await window.tronWeb.contract(heistABI3, contractAddress);
      ...
      TronHeistContract.latestRoundID().call().then(function(results){
        ...
        if(debug)
          showError("You appear to be on the wrong network!<br/><br/>Check that your Tron Wallet is connected to the Shasta Testnet and try again.","Wrong Network");
        else
          showError("You appear to be on the wrong network!<br/><br/>Check that your Tron Wallet is connected to the Mainnet and try again.","Wrong Network");
      });
  ...
  TronHeistContract.roundInfo(latestRoundID).call().then(function(results){
    ...
    if(currentRound.isDark){
      ...
      if(!darkSet){
        ...
        $('#heistRoomInner').removeClass('heistRoomLight').addClass('heistRoomDark');
        darkSet = true;
      }
    } else {
      ...
      if(darkSet){
        ...
        $('#heistRoomInner').removeClass('heistRoomDark').addClass('heistRoomLight'); 
        darkSet = false;
      }
    ...
    TronHeistContract.dailyInfo().call().then(function(results){
      ...
      TronHeistContract.instantWinJackpot().call().then(function(results){
        ...
        TronHeistContract.totalsInfo().call().then(function(results){
          // trx total distributed...
          
          updateInnerHTML('distributedTrxTotal', window.tronWeb.fromSun(results._totalDistributedReturns.add(results.totalJackpots.add(results.totalInstantWins.add(results.totalSafetyDeposits))).toString()).substring(0,8) + " TRX");
        });
...
function getInvestorInfo(_callback) {
  ...
  TronHeistContract.investorInfo(formatAddress(userWalletAddress),latestRoundID).call().then(function(results){
    ...
    if(results) {
      ...
      TronHeistContract.investorFinalkeyInfo(formatAddress(userWalletAddress)).call().then(function(results){
        //roundFinalChance
        if(new BigNumber(results.roundFinalKeys).eq(0)){
          updateInnerHTML('roundFinalChance', '-- -> --');
          updateInnerHTML('yourFinalKeysNumber', '0');
        } else {
          updateInnerHTML('yourFinalKeysNumber', new BigNumber(results.roundFinalKeys).div(DIVISOR).decimalPlaces(2).toString());
          updateInnerHTML('roundFinalChance', new BigNumber(results.roundFinalStartPercent).plus(1).div(1000).decimalPlaces(3).toString() + "->" + new BigNumber(results.roundFinalEndPercent).div(1000).decimalPlaces(3).toString());  
        }


        if(results.roundTotalFinalKeys.gt(0))
          updateInnerHTML('totalFinalKeys', new BigNumber(results.roundTotalFinalKeys).div(DIVISOR).decimalPlaces(2).toString());
        else 
          updateInnerHTML('totalFinalKeys', 0);
    ...
    //getting investorInfo: TypeError: Cannot read property 'tronWeb' of undefined
    TronHeistContract.latestRoundID().call().then(function(results){
      latestRoundID = parseInt(results.toNumber());
      console.log("UPDATING ROUND:", latestRoundID);
      getInvestorInfo();
...
function addEvent(event, isInit) {
  ...
  if(event.name == "KeysIssued") {
    ...
    if(event.result.instantWinResult == event.result.instantWinGuess) {
      ...
      playersRow += '<td>' + event.result.instantWinGuess + '</td><td >';

      if(parseInt(event.result.instantWinResult) < parseInt(event.result.instantWinGuess)){
        playersRow += '<i class="fas fa-chevron-down" style="color: #999;"></i>&nbsp;';
      } else {
        playersRow += '<i class="fas fa-chevron-up" style="color: #999;"></i>&nbsp;';
      }
      playersRow += event.result.instantWinResult + '</td>';
    }
    ...
    if(event.result.safetyDepositResult == 0){
      ...
      if(event.result.safetyDepositResult == event.result.safetyDepositGuess) {
        ...
        playersRow += '<td>' + event.result.safetyDepositGuess + '</td><td >';

        if(parseInt(event.result.safetyDepositResult) < parseInt(event.result.safetyDepositGuess)){
          playersRow += '<i class="fas fa-chevron-down" style="color: #999;"></i>&nbsp;';
        } else {
          playersRow += '<i class="fas fa-chevron-up" style="color: #999;"></i>&nbsp;';
        }
        playersRow += event.result.safetyDepositResult + '</td>';    
      }
    }
...
function updateClocks() {
  ...
  if(currentRound.isDark)
    triggerUpdateRate = 1; // update game info every 1s

//console.log("parseInt(minutesDifference)", parseInt(minutesDifference));
//console.log("parseInt(hoursDifference)", parseInt(hoursDifference));
  ...
  if(currentRound.isDark) {
    ...
    updateInnerHTML('roundCountdown', "HEIST PHASE!");
    updateInnerHTML('clock2', "Heist Phase");
  ...
  if(_diff < 0) {
    updateInnerHTML('dailyClock', "Day Ended - waiting next key purchase to award");    
  } else {
...
function confirmRoundRestart(){
  ...
  if( (difference < 1 && currentRound.roundID == lastGameRound) || currentRound.roundID > lastGameRound) {
    ...
    if(!hasShownRestartMsg) {
      ...
      _msg += "<strong>Buy Keys to Start the Next Round</strong>"
      showError(_msg, "Vault Prize WON!!!");
...
function dailyKey() {
  ...
  if(totalKeys.eq(0)){
    _canHaveFree = false;
    showError("You must buy at least 1 Key in each game round to open up the Free Daily Key bonus!<br/><br/>Buy your round key now to start earning and for a chance to win!", "Free Key Bonus");
    return;    
  }
  ...
  TronHeistContract.timeToFreeKey(userWalletAddress).call().then(function(results){
    ...
    showProcessing();
    let hasErr = false;
    TronHeistContract.dailyFreeKey(instantWinCode).send().catch(function(err){
      ...
      if(requiresRestart) {
        ...
        TronHeistContract.latestRoundID().call().then(function(results){
          ...
          if(_currentRoundID > currentRound.roundID) {
            ...
            TronHeistContract.finalizeAndRestart(_ref,instantWinCode,safetyDepositCode).send({
