...
window.addEventListener('load', function () {
  ...
  $('#loadingGif').attr('src', 'images/loading.gif'); //$("time.timeago").timeago();

  $("#previousPlayersTable").tableHeadFixer({
    head: true
  });
  $("#eventsTable").tableHeadFixer({
    head: true
  });
  $("#winsTable").tableHeadFixer({
    head: true
  });
  ...
  function setupTron() {
    if (window.tronWeb) {
      ...
      */
      ...
      var _tronWeb = new TronWeb(fullNode, solidityNode, eventServer, NULL_ADDR);

      window.tronWeb = _tronWeb;
    }

    console.log("Init Referer:", _referer); //alert(window.tronWeb.fromSun(50000000000));
...
function checkNetwork(_callback) {
  async function setupContracts() {
    ...
    try {
      TronHeistContract = await window.tronWeb.contract(heistABI3, contractAddress); //TronHeistWinners = await window.tronWeb.contract(winnersABI, winnersAddress)
    ...
    if (!isReadonly) {
      ...
      userWalletAddressShort = userWalletAddress.substring(0, 15) + "..."; //setNewCardBg();
    ...
    TronHeistContract.latestRoundID().call().then(function (results) {
      ...
      if (debug) showError("You appear to be on the wrong network!<br/><br/>Check that your Tron Wallet is connected to the Shasta Testnet and try again.", "Wrong Network");else showError("You appear to be on the wrong network!<br/><br/>Check that your Tron Wallet is connected to the Mainnet and try again.", "Wrong Network");
    });
  }
...
async function getPreviousRounds(_callback) {
  for (var _c = 0; _c < latestRoundID; _c++) {
    await TronHeistContract.roundInfo(_c).call().then(function (results) {
...
function getGameRound(_callback) {
  ...
  TronHeistContract.roundInfo(latestRoundID).call().then(function (results) {
    ...
    currentRound.safetyDepositPot = new BigNumber(results.safetyDepositPot); // needs to update the needsrestart state here!
    ...
    TronHeistContract.dailyInfo().call().then(function (results) {
      ...
      TronHeistContract.instantWinJackpot().call().then(function (results) {
        ...
        if (window.tronWeb.isAddress(userWalletAddress)) getInvestorInfo();
...
async function getDayPositions() {
  TronHeistContract.getCurrentDayChartSize().call().then(function (results) {
    var _currentDay = parseInt(results.day.toNumber());

    var _size = parseInt(results.size.toNumber()); //console.log("_currentDay:", _currentDay);
    ...
    async function processPositions() {
      var _loop = async function _loop(_c2) {
        await TronHeistContract.getDayChartAtPos(_currentDay, _c2).call().then(function (results) {
          console.log("C:", _c2);
...
function getInvestorInfo(_callback) {
  if (!window.tronWeb.isAddress(userWalletAddress)) return;
  TronHeistContract.investorInfo(formatAddress(userWalletAddress), latestRoundID).call().then(function (results) {
    ...
    if (results) {
      ...
      TronHeistContract.investorFinalkeyInfo(formatAddress(userWalletAddress)).call().then(function (results) {
        ...
        if (results.roundTotalFinalKeys.gt(0)) updateInnerHTML('totalFinalKeys', new BigNumber(results.roundTotalFinalKeys).div(DIVISOR).decimalPlaces(2).toString());else updateInnerHTML('totalFinalKeys', 0);
      });
    }
    ...
    console.log("ERR getting investorInfo:", err); //getting investorInfo: TypeError: Cannot read property 'tronWeb' of undefined
...
async function initGameRoundEvents() {
  ...
  var _maxPages = 10; //updatePrizes();

  console.log("initGameRoundEvents currentRound.startTime", currentRound.startTime.toNumber() * 1000); //1552563024000
  //1552582161000

  var _tableRowsAdded = 0;

  for (var page = 1; page < _maxPages + 1; page++) {
    page = await queryEventServer("KeysIssued", page, _maxPages);

    if (page == _maxPages) {
      console.log("END:", page); //displayHighestKeyholders();
...
async function queryEventServer(name, page, _maxPages, _sinceTimestamp) {
  var _url = "https://" + api_url + "/event/contract/" + contractAddress + "/" + name + "?size=";

  _url += "200&page=" + page.toString();
  if (_sinceTimestamp) _url += "&sinceTimestamp=" + _sinceTimestamp;
  _url += "&previousLastEventFingerprint=" + _fingerprint;
  _url += "&sort=block_timestamp"; //console.log(_url);
  ...
  await $.getJSON(_url, function (data) {
    ...
    if (data.length > 0) {
      ...
      for (var _c3 = 0; _c3 < data.length; _c3++) {
        if (data[_c3]._fingerprint) {
          _fpSet = true; //console.log("FP:", data[c]._fingerprint);

          _fingerprint = data[_c3]._fingerprint;
        }

        if ($.inArray(data[_c3].transaction_id + ":" + data[_c3].event_name + ":" + data[_c3].event_index, txSeen) == -1) {
          txSeen.push(data[_c3].transaction_id + ":" + data[_c3].event_name + ":" + data[_c3].event_index); // transform...

          var dataItem = {
            timestamp: data[_c3].block_timestamp,
            contract: data[_c3].contract_address,
            name: data[_c3].event_name,
            block: data[_c3].block_number
          };

          if (data[_c3].event_name == "KeysIssued") {
            dataItem.result = {
              keys: data[_c3].result.keys,
              rnd: data[_c3].result.rnd,
              timestamp: data[_c3].result.timestamp,
              to: "41" + data[_c3].result.to.substring(2),
              instantWinGuess: data[_c3].result.InstantWinGuess,
              instantWinAmount: data[_c3].result.InstantWinAmount,
              instantWinResult: data[_c3].result.InstantWinResult,
              safetyDepositGuess: data[_c3].result.SafetyDepositGuess,
              safetyDepositAmount: data[_c3].result.SafetyDepositAmount,
              safetyDepositResult: data[_c3].result.SafetyDepositResult
            };
          }

          if (data[_c3].event_name == "TrxDistributed") {
            dataItem.result = {
              amount: data[_c3].result.amount,
              rnd: data[_c3].result.rnd,
              timestamp: data[_c3].result.timestamp
            };
          }

          if (data[_c3].event_name == "AirdropWon" || data[_c3].event_name == "ReturnsWithdrawn" || data[_c3].event_name == "MegaFundWon" || data[_c3].event_name == "JackpotWon") {
            dataItem.result = {
              amount: data[_c3].result.amount,
              rnd: parseInt(data[_c3].result.rnd) + 1,
              timestamp: data[_c3].result.timestamp,
              by: "41" + data[_c3].result.by.substring(2) //console.log(dataItem);

            };
          }

          if (data[_c3].event_name == "DailyPrize") {
            dataItem.result = {
              timestamp: data[_c3].result.timestamp,
              day: parseInt(data[_c3].result.day) + 1,
              to: "41" + data[_c3].result.to.substring(2),
              amt: data[_c3].result.amt //console.log("----", data[c], dataItem);

            };
          }

          if (data[_c3].event_name == "RoundStarted") {
            dataItem.result = {
              timestamp: data[_c3].result.timestamp,
              round: parseInt(data[_c3].result.ID) + 1,
              hardDeadline: data[_c3].result.hardDeadline
            };
          }

          if (data[_c3].event_name == "PrizesUpdated") {
            dataItem.result = {
              timestamp: data[_c3].result.timestamp,
              SafetyDeposit: data[_c3].result.SafetyDeposit,
              InstantWin: data[_c3].result.InstantWin
            };
          ...
          var _keys = new BigNumber(data[_c3].result.keys).div(DIVISOR).toString();
          /*
...
async function initOtherEvents() {
  var _maxPages = 5;
  _fingerprint = ""; //updateInnerHTML('debugCode', "04");
...
function startEventListeners() {
  ...
  */
  ...
  TronHeistContract["KeysIssued"]().watch(function (err, res) {
    if (err) console.log("Err on event:", err);
    if (res) processLogResult(res);
  }).catch(function (err) {
...
function processLogResult(logEvent) {
  console.log("processLogResult:", logEvent); // update the UI

  getGameRound();
  addEvent(logEvent);

  if (logEvent.name == 'KeysIssued') {
    var playersRow = '<tr class="smallTxt">';

    var _keys = new BigNumber(logEvent.result.keys).div(DIVISOR).toString(); //console.log("BLOCK CHECK:" + results[c].blockNumber + ":" + roundStartBlock);
    //console.log("BLOCK DETAILS:" + results[c].returnValues.to +":" + _keys);


    if (parseInt(logEvent.result.rnd) == parseInt(latestRoundID)) {// is withiin current round so can be added to highest keyholder section
      //updateHighestKeyholders(logEvent.result.to, _keys);
      //getDayPositions();
    }

    playersRow += '<td>';
    playersRow += '<a href="https://tronscan.org/#/address/' + window.tronWeb.address.fromHex(logEvent.result.to) + '" target="_new">'; //playersRow += '<span class="hide-for-small-only hide-for-medium-only">'

    playersRow += formatAddress(logEvent.result.to); //playersRow += '</span>';
    ...
    var _logDate = new Date(logResults[c].result.timestamp * 1000).toISOString();
  ...
  if (logEvent.name == 'RoundStarted') {
    ...
    console.log(results); // latest = results.lenght -1;
...
function processInitEvents(isInit) {
  ...
  if (!isInit) console.log(initEvents);
  var initEventsSorted = initEvents.sort(function (obj1, obj2) {
    return parseInt(obj2.result.timestamp) - parseInt(obj1.result.timestamp);
  });

  for (var _c4 = 0; _c4 < initEventsSorted.length; _c4++) {
    try {
      addEvent(initEventsSorted[_c4], isInit);
    } catch (e) {}
  }

  if (isInit) getDayPositions(); //startEventListeners();

  $("time.timeago").timeago(); //updateInnerHTML('debugCode', "08b");
}

var _maxTimestamp = lastTimeStamp;
...
async function checkHeistEvents() {
  ...
  //startTimestamp = new Date().getTime();
  console.log("Checking Heist Events", startTimestamp); //updatePrizes();
  ...
  return; //updateInnerHTML('debugCode', "09b");
  ...
  await tronWeb.getEventResult(contractAddress, {
    ...
    if (res) {
      var _highestTimeStamp = 0; //console.log("RES:", res);

      for (var _c5 = 0; _c5 < res.length; _c5++) {
        if (eventsProcessed.includes(res[_c5].transaction + ":" + res[_c5].name)) {// dup
        } else {
          eventsProcessed.push(res[_c5].transaction + ":" + res[_c5].name);

          if (res[_c5].timestamp > lastTimeStamp) {
            if (parseInt(res[_c5].timestamp) >= parseInt(_highestTimeStamp)) {
              _highestTimeStamp = parseInt(res[_c5].timestamp);
            } //console.log(res);
  ...
  await tronWeb.getEventResult(winnersAddress, {
    sinceTimestamp: startTimestamp,
    //eventName: "KeysIssued",
    size: 200,
    page: 1
    ...
    if (res) {
      ...
      for (var _c6 = 0; _c6 < res.length; _c6++) {
        if (eventsProcessed.includes(res[_c6].transaction + ":" + res[_c6].name)) {// dup
        } else {
          eventsProcessed.push(res[_c6].transaction + ":" + res[_c6].name);

          if (res[_c6].timestamp > lastWinnersTimeStamp) {
            if (parseInt(res[_c6].timestamp) >= parseInt(_highestTimeStamp)) {
              _highestTimeStamp = parseInt(res[_c6].timestamp);
            } //console.log(res);
            //console.log("NEW EVENT:", res[c]);
...
function updateKeysIssued() {
  ...
  window.tronWeb.getEventResult(contractAddress, {
    ...
    //currentRound.startTime.toNumber() * 1000,
    eventName: "KeysIssued",
    size: 50,
    onlyConfirmed: true
  }).then(function (results) {
    for (var _c7 = 0; _c7 < results.length; _c7++) {
      if (results[_c7].timestamp > lastTimeStamp) {
        initEvents.push(results[_c7]);
        foundResults = true;
        if (results[_c7].timestamp > _maxTimestamp) _maxTimestamp = results[_c7].timestamp;
      } else {
        console.log("TIMESTAMP:", results[_c7].timestamp, lastTimeStamp); //console.log("OLD:", results[c])
        //1553085465000 
        //1553099634001
      }
    }

    if (foundResults) processInitEvents(false);else {
...
function addEvent(event, isInit) {
  ...
  var eventRow = '<tr class="smallTxt">';
  var _action = "";

  var _logDate = new Date(parseInt(event.result.timestamp) * 1000).toISOString();
  ...
  if (event.name == "KeysIssued") {
    ...
    var _keys = new BigNumber(event.result.keys).div(DIVISOR).toString(); //updateInnerHTML('debugCode', "10c");
    ...
    if (event.result.instantWinResult == event.result.instantWinGuess) {
      ...
      var _winRow = '<tr class="smallTxt"><td>';
      ...
      _winRow += '</tr>';
      if (!isInit) $('#winsTable tbody').prepend(_winRow);else $('#winsTable tbody').append(_winRow);

      var _winTable = document.getElementById('winsTable');
    ...
    if (event.result.safetyDepositResult == event.result.safetyDepositGuess) {
      ...
      var _winRow2 = '<tr class="smallTxt"><td>';
      _winRow2 += '<a href="https://tronscan.org/#/address/' + formatAddress(event.result.to) + '" target="_new">';
      _winRow2 += formatAddress(event.result.to).substring(0, 10);
      _winRow2 += '...</a> won ';
      _winRow2 += window.tronWeb.fromSun(event.result.safetyDepositAmount).substring(0.8) + ' TRX';
      _winRow2 += ' Safety Deposit Prize!</td>';
      _winRow2 += '<td><time class="timeago" datetime="' + _logDate + '">' + _logDate + '</time></td>';
      _winRow2 += '</tr>';
      if (!isInit) $('#winsTable tbody').prepend(_winRow2);else $('#winsTable tbody').append(_winRow2);

      var _winTable2 = document.getElementById('winsTable');
    ...
    playersRow += '<td><time class="timeago" datetime="' + _logDate + '">' + _logDate + '</time></td>';
    playersRow += '</tr>';
    if (!isInit) $('#previousPlayersTable tbody').prepend(playersRow);else $('#previousPlayersTable tbody').append(playersRow);
    var previousPlayersTable = document.getElementById('previousPlayersTable');
    var winTable = document.getElementById('winsTable');
  ...
  if (event.name == "JackpotWon") {
    ...
    } else _action += window.tronWeb.fromSun(event.result.amount) + " TRX Round [" + event.result.rnd + "] Jackpot Prize Won by " + formatAddress(event.result.by).substring(0, 10) + "...";
    ...
    var _winRow3 = '<tr class="smallTxt"><td>';
    _winRow3 += '<a href="https://tronscan.org/#/address/' + formatAddress(event.result.by) + '" target="_new">';
    _winRow3 += formatAddress(event.result.by).substring(0, 20);
    _winRow3 += '...</a> won ';
    _winRow3 += window.tronWeb.fromSun(event.result.amount).substring(0.8) + ' TRX';
    _winRow3 += ' Round [' + event.result.rnd + '] Jackpot!</td>';
    _winRow3 += '<td><time class="timeago" datetime="' + _logDate + '">' + _logDate + '</time></td>';
    _winRow3 += '</tr>';
    if (!isInit) $('#winsTable tbody').prepend(_winRow3);else $('#winsTable tbody').append(_winRow3);

    var _winTable3 = document.getElementById('winsTable');

    currentRows = _winTable3.getElementsByTagName("tr").length;
  ...
  if (event.name == "DailyPrize") {
    ...
    var _winRow4 = '<tr class="smallTxt"><td>';
    _winRow4 += '<a href="https://tronscan.org/#/address/' + formatAddress(event.result.to) + '" target="_new">';
    _winRow4 += formatAddress(event.result.to);
    _winRow4 += '...</a>  ';
    _winRow4 += window.tronWeb.fromSun(event.result.amt).substring(0.8) + ' TRX';
    _winRow4 += ' Daily Prize!</td>';
    _winRow4 += '<td><time class="timeago" datetime="' + _logDate + '">' + _logDate + '</time></td>';
    _winRow4 += '</tr>';
    if (!isInit) $('#winsTable tbody').prepend(_winRow4);else $('#winsTable tbody').append(_winRow4);

    var _winTable4 = document.getElementById('winsTable');
  ...
  eventRow += '</tr>';
  if (isInit) $('#eventsTable tbody').append(eventRow);else $('#eventsTable tbody').prepend(eventRow);
  ...
  if (!isInit) $("time.timeago").timeago();
}
...
function beatPosition(positionToBeat) {
  ...
  var _keys = parseInt(document.getElementById('leadersKeys' + positionToBeat).innerHTML) + 1; //_keys = _keys - totalKeys.toNumber();
...
function updateHighestKeyholders(_addr, _keys) {
  ...
  for (var _c8 = 0; _c8 < highestKeyholders.length; _c8++) {
    if (highestKeyholders[_c8].address == _addr) {
      highestKeyholders[_c8].keys = parseFloat(highestKeyholders[_c8].keys) + parseFloat(_keys);
      haveUpdated = true;
      break;
    }
  }
...
function resetHighestKeyHolders() {
  ...
  updateInnerHTML('trxInvested0', "-"); //updateInnerHTML('trxEarn0', "-");

  updateInnerHTML('leaders1', 'Available!');
  updateInnerHTML('leadersKeys1', '1');
  updateInnerHTML('trxInvested1', "-"); //updateInnerHTML('trxEarn1', "-");

  updateInnerHTML('leaders2', 'Available!');
  updateInnerHTML('leadersKeys2', '1');
  updateInnerHTML('trxInvested2', "-"); //updateInnerHTML('trxEarn2', "-");

  updateInnerHTML('leaders3', 'Available!');
  updateInnerHTML('leadersKeys3', '1');
  updateInnerHTML('trxInvested3', "-"); //updateInnerHTML('trxEarn3', "-");
...
function displayHighestKeyholders() {
  ...
  console.log(currentDayHolders); //console.log("SORTED:", sorted);
  ...
  for (var _c9 = 0; _c9 < sorted.length; _c9++) {
    totalKeys = totalKeys.plus(sorted[_c9].keys);
  }
  ...
  } catch (e) {} //updateInnerHTML('debugCode', "03b");
  ...
  for (var _c10 = 0; _c10 < highestKeyholders.length; _c10++) {
    //console.log("SORTED:", sorted[c]);
    var _keys = sorted[_c10].keys;

    if (_keys > 0) {
      try {
        var playersRow = '<tr class="smallTxt2">';
        playersRow += '<td style="width: 300px;">';
        playersRow += '<a href="https://tronscan.org/#/address/' + formatAddress(sorted[_c10].address) + '" target="_new">';
        if (isMobile) playersRow += formatAddress(sorted[_c10].address).substring(0, 16);else playersRow += formatAddress(sorted[_c10].address);
        playersRow += '</a>';
        playersRow += '</td>';
        playersRow += '<td><b>' + roundNumber(_keys, 2) + '</b></td>';

        var _keysTobeat = parseFloat(_keys) + 1;
...
function updateClocks() {
  ...
  var triggerUpdate2 = false;
  var triggerUpdateRate = 5;
  var triggerUpdateRate2 = 15;
  if (currentRound.isDark) triggerUpdateRate = 1; // update game info every 1s
  ...
  if (currentRound.isDark) {
    ...
    if (currentRound.softDeadline.toNumber() > 0) {
      ...
      if (orig_diff < 1) {
        ...
        if (daysDifference > 0) {
          if (daysDifference == 1) _diffStr = "1 day & ";else _diffStr = daysDifference + " days, ";
        }
  ...
  var _now = new Date().getTime(); //if(now.sub(gameDays[currentDay].dayStartTs).div(dayDuration) > 0) {


  var _endTime = currentRound.dayStartTs.times(1000).toNumber() + 60 * 60 * 24 * 1000;

  var _diff = _endTime - _now;

  if (_diff < 0) {
    updateInnerHTML('dailyClock', "Day Ended - waiting next key purchase to award");
  } else {
    var _hrs = Math.floor(_diff / 1000 / 60 / 60);

    _diff -= _hrs * 1000 * 60 * 60;

    var _mns = Math.floor(_diff / 1000 / 60);

    _diff -= _mns * 1000 * 60;

    var _scs = Math.floor(_diff / 1000);

    var _diffStr2 = padNumber(_hrs, 2) + ":" + padNumber(_mns, 2) + ":" + padNumber(_scs, 2);
...
function confirmRoundRestart() {
  ...
  if (difference < 1 && currentRound.roundID == lastGameRound || currentRound.roundID > lastGameRound) {
    ...
    if (!hasShownRestartMsg) {
      var _msg = "<h5>The Round Has Been Won!</h5>";
      ...
      _msg += "The final key holder will be confirmed on the smart contract, where a winner will be selected at random based on the number of <strong>Final Keys</strong held - and when the next round starts the funds will be transferred into their winnings!<br/><br/>";
...
function checkForRandomTx() {
  if (tx_to_check.length > 0) {
    for (var _c11 = 0; _c11 < tx_to_check.length; _c11++) {}
  }
}
...
function dailyKey() {
  ...
  TronHeistContract.timeToFreeKey(userWalletAddress).call().then(function (results) {
    var _canHaveFree = true; //console.log(results._timeToFreeKey.toNumber());

    var _timeRemaining = parseInt(results._timeToFreeKey.toNumber()); //console.log("_timeRemaining", _timeRemaining);
    ...
    if (_timeRemaining < 86400) {
      ...
      if (currentRound.isDark == true) {
        ...
        if (currentRound.hardDeadline > new Date().getTime()) {
          // TOTEST
          _canHaveFree = false;
    ...
    var _instantWinCode = document.getElementById('instantWinCode').value;
    var instantWinCode = 300;

    if (isNaN(_instantWinCode)) {} else {
    ...
    TronHeistContract.dailyFreeKey(instantWinCode).send().catch(function (err) {
      ...
      hideProcessing();
      console.log("results:", results); // result = tx 6dfa249ccd7793d7b4c87a08f5ed44ec7eefd7a109d95a18bdb02d28136b72b4
...
function processReinvest() {
  ...
  var hasErr = false;
  TronHeistContract.minInvestment().call().catch(function (err) {}).then(function (results) {
    if (hasErr) return;
    minPlayAmount = new BigNumber(results);
    var investmentAmount = document.getElementById('investmentAmount').value;
    var investmentAmountTRX = new BigNumber(keysPrice.times(investmentAmount));
    var minPlayAmountTRX = new BigNumber(window.tronWeb.fromSun(minPlayAmount.toString()));
    var _instantWinCode = document.getElementById('instantWinCode').value;
    var instantWinCode = 300;
    ...
    var _safetyDepositCode = document.getElementById('safetyDepositCode').value;
    var safetyDepositCode = 150;

    if (isNaN(_safetyDepositCode)) {} else {
    ...
    if (investmentAmountTRX.isLessThan(minPlayAmountTRX)) {
      ...
      if (requiresRestart) {
        ...
        TronHeistContract.reinvestReturns(window.tronWeb.toSun(investmentAmountTRX), _ref, instantWinCode, safetyDepositCode).send({// shouldPollResponse: true
        }).catch(function (err) {
          _hasErr = true;
          hideProcessing();
          console.log("error", err);
...
function processWithdraw() {
  ...
  TronHeistContract.unclaimedReturns().call().then(function (results) {
    var _maxReturnsNow = new BigNumber(results.toString());
  ...
  TronHeistContract.minInvestment().call().catch(function (err) {}).then(function (results) {
    ...
    if (investmentAmountTRX.isLessThan(minPlayAmountTRX)) {
      ...
      if (requiresRestart) {
        ...
        console.log("currentRound.roundID", currentRound.roundID); // check if the round has already been restarted...

        TronHeistContract.latestRoundID().call().then(function (results) {
          var _currentRoundID = parseInt(results.toNumber());
          ...
          if (_currentRoundID > currentRound.roundID) {
            ...
            requiresRestart = true; //showError("Please wait for the next (BIGGER!) round to start!", "Next Round Pending");
            //return;
...
function showNotConnectedMsg() {
  ...
  var isAndroid = false;
  if (navigator.userAgent.toLowerCase().indexOf("android") > -1) isAndroid = true;
