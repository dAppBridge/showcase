...
var contractAddress = "TVbUz7ZgDMAyZSaKKbUeaghBEg1pt38w9W";
var tronWinContractAddress = "TPGk9UrcGTXbipNzuj5ZXH2y96Zc626QSg";
var tronWinVaultAddress = "TDzhXaqzr2CU5PmFsCqZ5TH3UKntTzGNhb";
var tokenContractAddress = "TUaRbt26TdLoV6NCxpfUqkXxwYa3q7PMeP"; //9023.7 TRX
...
App = {
  ...
  initWeb3: async function initWeb3() {
    if (window.tronWeb) {
      // need to check they are on the right network
      App.isReadonly = false;
      App.tronWeb = window.tronWeb; //console.log("ADDR:", App.tronWeb.address.fromHex("0x93e05fdf63dea1a238a4bfdd69e90329999290d3"));
    } else {
  ...
  initContract: async function initContract() {
    if (!App.isReadonly) {
  ...
  bindEvents: function bindEvents() {
    ...
    $("#seedsTable").tableHeadFixer({
      head: true
    }); //getRoundKeyPrice();

    updateGameStats(true);
    loadInitVanityNames(getInitEvents);
    clipboard = new ClipboardJS('.btn'); //    $("#leaderboardTable").tableHeadFixer({head: true});

    console.log("INIT GOOGLE TRANS");
    new google.translate.TranslateElement({
      pageLanguage: "en, ru, zh-CN"
    ...
    $('#flag-english').click(function () {
      updateLang('en'); //setEng();
    });
    $('#flag-chinese').click(function () {
      updateLang('zh-CN');
    });
    $('#flag-russian').click(function () {
      updateLang('ru'); //setRussian();
    });

    var _browserLang = navigator.language || navigator.userLanguage;
    ...
    if (_browserLang.toUpperCase().indexOf('RU') > -1) {
      ...
      if (_lang.length > 0) {
        setTimeout(function () {
          //updateLang(_lang);
          if (_lang == "ru") //setRussian();
            updateLang('ru');else updateLang(_lang);
        }, 1000);
      }
    }
    ...
    $('#freezeBtn').click(function () {
      App.contracts.TronToken.balanceOfFrozen(App.userWalletAddress).call().then(function (results) {
        var _playerTokens = results.toNumber() / 1000000;

        $('#playerFrozen').html(_playerTokens);
        App.contracts.TronToken.totalFrozen().call().then(function (results) {
          var _totalFrozen = results.toNumber() / 1000000;

          $('#totalFrozen').html(_totalFrozen);

          if (tokenDayDetail.fundsAvail > 0) {
            $('#playerFrozenDivs').html(App.tronWeb.fromSun(_playerTokens / _totalFrozen * (tokenDayDetail.fundsAvail * 0.8)));
          }
        });
      });
    ...
    $('#unfreezeBtn').click(function () {
      App.contracts.TronToken.balanceOfFrozen(App.userWalletAddress).call().then(function (results) {
        ...
        App.contracts.TronToken.totalFrozen().call().then(function (results) {
          ...
          if (tokenDayDetail.fundsAvail > 0) {
            $('#playerFrozenDivs2').html(App.tronWeb.fromSun(_playerTokens / _totalFrozen * (tokenDayDetail.fundsAvail * 0.8)));
          }
        });
      });
...
function populatePreviousSeeds(_pos, _max) {
  App.contracts.TronWinDice.playerPreviousHash(App.userWalletAddress, _pos).call().then(function (results) {
    var _logDate = new Date(parseInt(results.startTS.toString()) * 1000).toISOString();

    var _row = '<tr><td><b>Server Seed Hashed</b><br/>' + results.serverHashHashed + '<br/>';
...
function setRussian() {
  ...
  console.log("SETTING RUSSIAN LANG"); //return;
...
function setEng() {
  ...
  console.log("SETTING RUSSIAN ENG"); //return;
...
function updateStats(_rollNumber) {
  ...
  if (App.rollTypeUnder) {
    var _multiplier = parseFloat((99 * 1000 / _rollNumber / 1000).toFixed(2));
    ...
    updateInnerHTML('payout', _payout.toFixed(3)); //debugCalc(_rollNumber);
  } else {
    var _multiplier2 = parseFloat((99 * 1000 / (99 - _rollNumber) / 1000).toFixed(2));

    updateInnerHTML('multiplier', 'x' + _multiplier2);
    updateInnerHTML('winChance', ((99 - _rollNumber) / 100 * 100).toFixed(0) + '%');
    _payout = document.getElementById('betAmount').value * _multiplier2;
    updateInnerHTML('payout', _payout.toFixed(3)); //debugCalc(_rollNumber);
  }
...
function setupRangeSlider() {
  ...
  var rangeSlider2 = function rangeSlider2() {
    ...
    slider.each(function () {
      ...
      range.on('input', function () {
        ...
        updateBetSize(this.value); //  updateStats(this.value);
        //  $(this).next(value).html(this.value);
        // resetSelectedRoll();
      });
    });
...
function diceRolling() {
  ...
  _dice1++;
  _dice2++;
  if (_dice1 > 9) _dice1 = 0;
  if (_dice2 > 9) _dice2 = 0;
...
function updateVanity() {
  ...
  var _name = document.getElementById('vanityNameInput').value;
  console.log("_NAME", _name);

  var _cost = App.tronWeb.toSun(100); // check if name exists...
...
function submitBuyVanity(_name) {
  var _cost = App.tronWeb.toSun(100);

  showToast('Sending Transaction', 'Check your Tron Wallet to complete the transaction', 'info'); // Working...
  ...
  App.contracts.TronWin.buyVanity(_name).send({
    ...
    if (err) {
      ...
      var _msg = "<h4>Congratulations!</h4>";
...
function updateClientSeed() {
  var _newSeed = $('#newClientSeed').val();
...
function getUserWalletAddress() {
  if (!App.isReadonly) {
    App.userWalletAddress = App.tronWeb.defaultAddress.hex; // now check for vanity..
...
function getInitEvents() {
  ...
  */
  ...
  if (1 == 1 || isMobile) {
    ...
    App.contracts.TronWinDice.Action().watch(function (err, event) {
    ...
    App.contracts.TronWin.Action().watch(function (err, event) {
      if (err) return console.error('Error with "method" event:', err);
...
var _fingerprint = "";
var api_url;
if (isDebug) api_url = "api.shasta.trongrid.io";else api_url = "api.trongrid.io";
    ...
    if (_data.length > 0) {
      for (var c = 0; c < _data.length; c++) {
        if (_data[c]._fingerprint) {
          _fpSet = true; //console.log("FP:", _data[c]._fingerprint);
        ...
        if ($.inArray(_data[c].transaction_id + ":" + _data[c].event_name + ":" + _data[c].event_index, txSeen) == -1) {
          ...
          console.log("DATAITEM:", dataItem); //initEvents.push(dataItem);
...
function processEvent(event, isInit, isMobileEventsCheck) {
  ...
  var eventRow = '';
  var hasEventRow = false;

  var _logDate = new Date(parseInt(event.result.timestamp) * 1000).toISOString();

  if ((isInit || isMobileEventsCheck) && event.blockNumber > highestBlocknumber) highestBlocknumber = event.block;

  if (event.name == "Action") {
    if (event.result.event_type == 99) {
      console.log("NEW EVENT:", event);
      eventRow = '<tr class="standardEventRow">';
      if (isMobile) eventRow += '<td>' + displayWalletAddress(event.result.from).substring(0, 10) + '</td>';else eventRow += '<td>' + displayWalletAddress(event.result.from) + '</td>';
      eventRow += '<td>Debug Dist: ' + App.tronWeb.fromSun(event.result.amnt) + '  |' + event.result.instant_guess + ':' + event.result.instant_result + ':' + event.result.instant_prize + ':' + event.result.bonus_guess + '</td>';
      hasEventRow = true;
    }
    ...
    if (event.result.event_type == 0 || event.result.event_type == 1) {
      ...
      if (!isInit && displayWalletAddress(event.result.from) == displayWalletAddress(App.userWalletAddress)) {
        // match
        var _result = parseInt(event.result.result);

        var _win_amnt = parseInt(event.result.win_amnt);
      ...
      if (isMobile) eventRow += '<td>' + displayWalletAddress(event.result.from).substring(0, 15) + '</td>';else eventRow += '<td>' + displayWalletAddress(event.result.from) + '</td>';
      if (event.result.bet_under == 'true') eventRow += '<td>Under ' + event.result.roll_amnt + '';else eventRow += '<td>Over ' + event.result.roll_amnt + '';
      eventRow += '</td>';
      hasEventRow = true;
      eventRow += '<td>' + event.result.result + '</td>';
      eventRow += '<td>' + displayFromSUN(event.result.amnt) + ' TRX</td>';
      if (_winRow) eventRow += '<td class="winAmntCol">' + displayFromSUN(event.result.win_amnt) + ' TRX</td>';else eventRow += '<td>' + displayFromSUN(event.result.win_amnt) + ' TRX</td>';
      eventRow += '</tr>';
    }
  } else {}
  ...
  if (hasEventRow) {
    ...
    if (!isInit) {
      $('#eventsTable tbody').prepend(eventRow); //  $("time.timeago").timeago();
    } else {
...
async function loadInitVanityNames(_callback) {
  App.contracts.TronWin.playersVanityAddressListLen().call().then(function (results) {
    playerVanityLen = parseInt(results);
    if (playerVanityLen > 0) getPlayerVanityByID(0, _callback);else {
...
async function getPlayerVanityByID(_c, _callback) {
  await App.contracts.TronWin.playersVanityByID(_c).call().then(function (results) {
...
function updateGameStats(isInit) {
  App.contracts.TronWinDice.gameStats().call().then(function (results) {
    ...
    App.contracts.TronWinVault.tokenDay().call().then(function (results) {
      var _tokenDay = parseInt(results.toString());
...
function doFreeze() {
  ...
  var _amnt = document.getElementById('freezeNum').value * 1000000;
  ...
  App.contracts.TronToken.freeze(_amnt).send({
    ...
    if (err) {
      ...
      var _msg = "<h4>Freeze Request Submitted!</h4>";
...
function doUnFreeze() {
  ...
  var _amnt = document.getElementById('unfreezeNum').value * 1000000;
  ...
  App.contracts.TronToken.unfreeze(_amnt).send({
    ...
    if (err) {
      ...
      var _msg = "<h4>Un-Freeze Request Submitted!</h4>";
...
function roll() {
  var numSUN = parseInt(App.tronWeb.toSun(document.getElementById('betAmount').value).toString()).toFixed(0); //console.log("numSUN", numSUN);
  ...
  var _rollNumber = $('#rangeSelector').val();
  ...
  if (App.rollTypeUnder) {
    ...
    var _multiplier3 = parseFloat((99 * 1000 / (99 - _rollNumber) / 1000).toFixed(2));

    _payout = document.getElementById('betAmount').value * _multiplier3;
  }
...
function calcMaxBet() {
  if (App.isReadonly) {
    var results = App.tronWeb.toSun(10000);
    playerBalance = new App.tronWeb.BigNumber(results.toString());

    var _currentBalanceTRX = App.tronWeb.fromSun(results);
    ...
    if (App.rollTypeUnder) {
      ...
      var _multiplier4 = parseFloat((99 * 1000 / (99 - _rollNumber) / 1000).toFixed(2));

      _maxBet = parseFloat(App.tronWeb.fromSun(gameStats._maxWin.toNumber())) / _multiplier4;
    }
    ...
    App.tronWeb.trx.getBalance(App.userWalletAddress).then(function (results) {
      ...
      if (App.rollTypeUnder) {
        var _multiplier5 = parseFloat((99 * 1000 / _rollNumber / 1000).toFixed(2));

        _maxBet = parseFloat(App.tronWeb.fromSun(gameStats._maxWin.toNumber())) / _multiplier5;
      } else {
        var _multiplier6 = parseFloat((99 * 1000 / (99 - _rollNumber) / 1000).toFixed(2));

        _maxBet = parseFloat(App.tronWeb.fromSun(gameStats._maxWin.toNumber())) / _multiplier6;
...
window.addEventListener('load', function () {
  ...
  if (_newReferer.length > 0) {
    ...
    if (_cookieRef.length > 0) _referer = _cookieRef;
  }
...
function displayWalletAddress(_addr) {
  var _out = formatAddr(_addr); //console.log(vanityNames, _out);
...
function updateLang(_newLang) {
  ...
  for (var i = 0; i < selectField.children.length; i++) {
    var option = selectField.children[i]; // find desired langauge and change the former language of the hidden selection-field 

    if (option.value == _newLang) {
      selectField.selectedIndex = i; // trigger change event afterwards to make google-lib translate this side
...
function setGamePause() {
  App.contracts.TronWin.p_setGamePaused(false).send({}, function (error, result) {
    if (error) {
...
function finalizeBet() {
  App.contracts.TronWinDice.finalizeBet().send({}, function (error, result) {
    if (error) {
      showError("Unable to access game contract - please try again.  If this issue persists try reloading this page.");
    } else {
      console.log("tx:", result);
...
function pushDataToRandos() {
  var _body = JSON.parse(_randosData);

  var _NewRandos = [];

  for (var i = 0; i < _body.data.length; i++) {
    _NewRandos.push(_body.data[i]);
  } //App.contracts.Rando2.pushRandos(_NewRandos).send(


  App.contracts.TronWinDice.pushRandos(_NewRandos).send({}, function (error, result) {
    if (error) {
      showError("Unable to access game contract - please try again.  If this issue persists try reloading this page.");
    } else {
      console.log("tx:", result);
...
function debug_genResult() {
  App.contracts.TronWinDice.debug_genResult().send({
    shouldPollResponse: true
  }).then(function (results) {
...
function getPlayerHashAtPos() {
  ...
  App.contracts.TronWinDice.getPlayerHashAtPos(App.userWalletAddress, 15).call().then(function (results) {
    console.log("_hexString:", results._hexString);
    console.log("_hash:", results._hash);
    console.log("_result:", results._result.toNumber());
  });
...
!function (t, e) {
  ...
  return function (n) {
    ...
    n.node = function (t) {
      return void 0 !== t && _instanceof(t, HTMLElement) && 1 === t.nodeType;
