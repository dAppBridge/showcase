...
function getGameRound(_callback) {
  TronHeistContract.roundInfo(latestRoundID).call().then(function (results) {
    ...
    currentRound.roundID = latestRoundID; //megaJackpot = new BigNumber(results._megaJackpot);
    ...
    updateInnerHTML('vaultPrize', window.tronWeb.fromSun(currentRound.jackpot.toString()).substring(0, 8) + " TRX"); ////updateInnerHTML('roundAirdropTotal', window.tronWeb.fromSun(currentRound.airdrop.toString()).substring(0,8) + " TRX");
    ...
    updateInnerHTML('roundHardDeadline', timeConverter(currentRound.hardDeadline)); //updateInnerHTML('megaJackpotTotal', window.tronWeb.fromSun(megaJackpot.toString()).substring(0,8) + " TRX");
    //console.log("GameRound:", results);
    //console.log("latestRoundID", latestRoundID);
    //console.log("currentRound.jackpot.toString()", currentRound.jackpot.toString());
    // get investor info

    if (window.tronWeb.isAddress(userWalletAddress)) getInvestorInfo(); //updateChanceRates();
...
async function initGameRoundEvents() {
  ...
  for (var page = 1; page < _maxPages + 1; page++) {
    ...
    if (page == _maxPages) {
      ...
      displayHighestKeyholders(); // start event listenders
      //startEventListeners();
...
async function queryEventServer(name, page, _maxPages, _sinceTimestamp) {
  var _url = "https://api.trongrid.io/event/contract/TEvnnwgSKvqKi3ZYJ1Sr91cyf48PsM8Q5f/" + name + "?size=";
  ...
  await $.getJSON(_url, function (data) {
    ...
    if (data.length > 0) {
      ...
      for (var _c2 = 0; _c2 < data.length; _c2++) {
        if (data[_c2]._fingerprint) {
          _fpSet = true; //console.log("FP:", data[c]._fingerprint);

          _fingerprint = data[_c2]._fingerprint;
        }

        if ($.inArray(data[_c2].transaction_id + ":" + data[_c2].event_name, txSeen) == -1) {
          txSeen.push(data[_c2].transaction_id + ":" + data[_c2].event_name); // transform...

          var dataItem = {
            timestamp: data[_c2].block_timestamp,
            contract: data[_c2].contract_address,
            name: data[_c2].event_name,
            block: data[_c2].block_number
          };

          if (data[_c2].event_name == "KeysIssued") {
            dataItem.result = {
              keys: data[_c2].result.keys,
              rnd: data[_c2].result.rnd,
              timestamp: data[_c2].result.timestamp,
              to: "41" + data[_c2].result.to.substring(2)
            };
          }

          if (data[_c2].event_name == "TrxDistributed") {
            dataItem.result = {
              amount: data[_c2].result.amount,
              rnd: data[_c2].result.rnd,
              timestamp: data[_c2].result.timestamp
            };
          }

          if (data[_c2].event_name == "AirdropWon" || data[_c2].event_name == "ReturnsWithdrawn" || data[_c2].event_name == "MegaFundWon") {
            dataItem.result = {
              amount: data[_c2].result.amount,
              rnd: data[_c2].result.rnd,
              timestamp: data[_c2].result.timestamp,
              by: "41" + data[_c2].result.by.substring(2)
            };
          ...
          var _keys = new BigNumber(data[_c2].result.keys).div(DIVISOR).toString();

          if (parseInt(data[_c2].result.rnd) == parseInt(latestRoundID) && name == "KeysIssued") {
            // is withiin current round so can be added to highest keyholder section
            updateHighestKeyholders(data[_c2].result.to, _keys);
          }
        } else {}
      }
    } else {
...
function processInitEvents(isInit) {
  ...
  for (var _c3 = 0; _c3 < initEventsSorted.length; _c3++) {
    try {
      addEvent(initEventsSorted[_c3], isInit);
    } catch (e) {}
  } //startEventListeners();
...
async function updatePrizes() {
  await TronHeistWinners.getPrizes().call().then(function (result) {
    ...
    updateInnerHTML('roundAirdropTotal2', tronWeb.fromSun(currentAirdropPrize.toNumber()).substring(0, 8)); //console.log("prizes:", tronWeb.fromSun(currentMegaPrize.toNumber()), tronWeb.fromSun(currentAirdropPrize.toNumber()));
  });
}
...
async function checkHeistEvents() {
  ...
  await tronWeb.getEventResult("TEvnnwgSKvqKi3ZYJ1Sr91cyf48PsM8Q5f", {
    ...
    if (res) {
      ...
      for (var _c4 = 0; _c4 < res.length; _c4++) {
        if (eventsProcessed.includes(res[_c4].transaction + ":" + res[_c4].name)) {// dup
        } else {
          eventsProcessed.push(res[_c4].transaction + ":" + res[_c4].name);

          if (res[_c4].timestamp > lastTimeStamp) {
            if (parseInt(res[_c4].timestamp) >= parseInt(_highestTimeStamp)) {
              _highestTimeStamp = parseInt(res[_c4].timestamp);
            } //console.log(res);
  ...
  await tronWeb.getEventResult("TUDMjoXCQJ9JBfCqfJGuxHF2autQ9u6Tt7", {
    sinceTimestamp: startTimestamp,
    //eventName: "KeysIssued",
    size: 200,
    page: 1
    ...
    if (res) {
      ...
      for (var _c5 = 0; _c5 < res.length; _c5++) {
        if (eventsProcessed.includes(res[_c5].transaction + ":" + res[_c5].name)) {// dup
          ...
          if (res[_c5].timestamp > lastWinnersTimeStamp) {
...
function updateKeysIssued() {
  ...
  window.tronWeb.getEventResult(contractAddress, {
    ...
    for (var _c6 = 0; _c6 < results.length; _c6++) {
      if (results[_c6].timestamp > lastTimeStamp) {
        initEvents.push(results[_c6]);
        foundResults = true;
        if (results[_c6].timestamp > _maxTimestamp) _maxTimestamp = results[_c6].timestamp;
      } else {
        console.log("TIMESTAMP:", results[_c6].timestamp, lastTimeStamp); //console.log("OLD:", results[c])
        //1553085465000 
        //1553099634001
      }
    }
...
function addEvent(event, isInit) {
  ...
  if (event.name == "KeysIssued") {
    ...
    var _logDate2 = new Date(parseInt(event.result.timestamp) * 1000).toISOString();

    playersRow += '<td><time class="timeago" datetime="' + _logDate2 + '">' + _logDate2 + '</time></td>';
    playersRow += '</tr>';
    if (!isInit) $('#previousPlayersTable tbody').prepend(playersRow);else $('#previousPlayersTable tbody').append(playersRow);
    var previousPlayersTable = document.getElementById('previousPlayersTable');
    var _currentRows = previousPlayersTable.getElementsByTagName("tr").length;

    if (_currentRows > 200) {
      previousPlayersTable.deleteRow(_currentRows - 1);
    }
  ...
  var _logDate = new Date(event.result.timestamp * 1000).toISOString();
  ...
  var currentRows = eventsTable.getElementsByTagName("tr").length;
...
function beatPosition(positionToBeat) {
  ...
  var _keys = 0;
  if (sorted[positionToBeat]) _keys = parseInt(sorted[positionToBeat].keys) + 1;else _keys = 1;
  $.toast({
...
function updateHighestKeyholders(_addr, _keys) {
  ...
  for (var _c7 = 0; _c7 < highestKeyholders.length; _c7++) {
    if (highestKeyholders[_c7].address == _addr) {
      highestKeyholders[_c7].keys = parseFloat(highestKeyholders[_c7].keys) + parseFloat(_keys);
      haveUpdated = true;
      break;
    }
  }
...
function displayHighestKeyholders() {
  ...
  for (var _c8 = 0; _c8 < highestKeyholders.length; _c8++) {
    var _keys = sorted[_c8].keys;

    if (_keys > 0) {
      try {
        var playersRow = '<tr class="smallTxt2">';
        playersRow += '<td style="width: 300px;">';
        playersRow += '<a href="https://tronscan.org/#/address/' + formatAddress(sorted[_c8].address) + '" target="_new">';
        if (isMobile) playersRow += formatAddress(sorted[_c8].address).substring(0, 16);else playersRow += formatAddress(sorted[_c8].address);
        playersRow += '</a>';
        playersRow += '</td>';
...
function updateClocks() {
  ...
  if (currentRound.softDeadline > 0) {
    ...
    var difference = currentRound.softDeadline * 1000 - dateNow; //console.log(difference);
...
function checkForRandomTx() {
  if (tx_to_check.length > 0) {
    for (var _c9 = 0; _c9 < tx_to_check.length; _c9++) {}
  }
}
  ...
  TronHeistContract.minInvestment().call().catch(function (err) {}).then(function (results) {
    ...
    if (investmentAmountTRX.isLessThan(minPlayAmountTRX)) {
      ...
      if (requiresRestart) {
        ...
        TronHeistContract.reinvestReturns(window.tronWeb.toSun(investmentAmountTRX)).send({// shouldPollResponse: true
        }).catch(function (err) {
          _hasErr = true;
          hideProcessing();
