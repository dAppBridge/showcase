...
contract TRC20 is ITRC20 {
    ...
    // freezing tokens...
    mapping(address => uint) tokensFrozen;
...
contract EWN is TRC20 {
    ...
    constructor () public {
        ...
        release_group_price[1][1] = 50000000000000000; // 0.05 eth
    ...
    function debug_testDificulty(uint256 tokens) public view returns(uint _difficultyRelease, uint _difficultyGroup) {
    ...
    function initTransfer(address to, uint256 value) external onlyTokenBanker() returns (bool) {
    ...
    function tokensFrozenByplayersWithFrozenTokensAddressesPos(uint256 _c) public view returns(uint) {
        return tokensFrozen[playersWithFrozenTokensAddresses[_c]];
    }

    function freeze(address _player, uint _amnt) public onlyTokenBanker() {

        uint _balance = balanceOf(_player);
        require(_balance > _amnt.add(tokensFrozen[_player]), "Not enough tokens!");

        tokensFrozen[_player] = tokensFrozen[_player].add(_amnt);
        if(playersWithFrozenTokens[_player] == 0){
            playersWithFrozenTokensAddresses.push(_player);
            playersWithFrozenTokens[_player] = playersWithFrozenTokensAddresses.length-1;
        }
        
    }

    function unfreeze(address _player, uint _amnt) public onlyTokenBanker() {

        require(tokensFrozen[_player] >= _amnt, "Not enough tokens frozen!");
        pendingUnfreezes.push(tokensToUnfreeze(_player, _amnt, now));
    }

    function pendingUnfreezesLen() public view returns (uint) {
        return pendingUnfreezes.length;
    }
    function pendingUnfreezesAtPos(uint _pos) public onlyTokenBanker() returns(address _player, uint _amnt, uint _startTS)  {
        ...
        delete pendingUnfreezes[pendingUnfreezes.length-1];
        pendingUnfreezes.length--;
        if(tokensFrozen[_player] == 0){
            delete playersWithFrozenTokensAddresses[playersWithFrozenTokens[_player]];
