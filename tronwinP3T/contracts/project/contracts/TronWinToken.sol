...
*
*  PLAY AND EARN TWN AT: https://tronwin.app/
*  
* --- TRON WIN TWN TOKEN -----------------------------------------------
*
* 
...
contract TWN is TRC20 {
    ...
    bool public initTWNtransferComplete = false;

    constructor () public {
        contract_owner = msg.sender;
        tokenBanker = contract_owner;
        _name = "TWN";
        _symbol = "TWN";
        _decimals = 6;
        ...
        release_group_price[1][2] = 441000000;
        release_group_price[1][3] = 442000000;
        release_group_price[1][4] = 443000000;
        release_group_price[1][5] = 444000000;
        release_group_price[1][6] = 445000000;
        release_group_price[1][7] = 446000000;
        release_group_price[1][8] = 447000000;
        release_group_price[1][9] = 448000000;
        
        playersWithFrozenTokensAddresses.push(address(0));
    }

    // used to transfer existing TWN contract data over
    // one time event before _this_ contract is made live
    function transferTWN(address _owner, uint _amnt, uint _amntFrozen) public onlyOwner {

        require(initTWNtransferComplete == false, "Already completed init transfers!");

        transfer(_owner, _amnt);

        tokensInPlay = tokensInPlay + _amnt;

        tokensFrozen[_owner] = _amntFrozen;
        totalFrozen = totalFrozen + _amntFrozen;
        if(playersWithFrozenTokens[_owner] == 0){
            playersWithFrozenTokensAddresses.push(_owner);
            playersWithFrozenTokens[_owner] = playersWithFrozenTokensAddresses.length-1;
        }
    }
    function completeInitTransfers() public onlyOwner {
        initTWNtransferComplete = true;
    }
    ...
    function initTransfer(address to, uint256 valueSpent) external onlyTokenBanker() returns (bool) {
        ...
        tokensInPlay = tokensInPlay + value;
    ...
    function freeze(uint _amnt) public {
        ...
        uint _balance = fullBalanceOf(msg.sender);
    ...
    function burn(uint256 value) public {
        require(value <= balanceOf(msg.sender), "Insufficient Balance or Too Many Frozen to complete burn!");
    ...
    function burnFrom(address from, uint256 value) public {
        require(value <= balanceOf(from), "Insufficient Balance or Too Many Frozen to complete burn!");
    ...
    function transfer(address to, uint256 value) public returns (bool) {
        require(value <= balanceOf(msg.sender), "Insufficient Balance or Too Many Frozen to complete transfer!");
    ...
    {
        require(addedValue.add(allowance(msg.sender,spender)) <= balanceOf(msg.sender), "Insufficient Balance or Too Many Frozen to complete approval!");

        super.increaseAllowance(spender, addedValue);
    }
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        require(value <= balanceOf(from), "Insufficient Balance or Too Many Tokens to complete transfer!");
        super.transferFrom(from, to, value);
    }
    function balanceOf(address owner) public view returns (uint256) {
        uint _balance = super.balanceOf(owner).sub(tokensFrozen[owner]);
        return _balance;
    }
    function fullBalanceOf(address owner) public view returns (uint256) {
    ...
    function updateContractOwner(address _addr) public onlyOwner {
