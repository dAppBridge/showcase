...
contract TronWinDice {
    ...
    function roll(uint _rollAmnt, bool _isUnder) public payable validPlayer notOnPause {
        ...
        (_win_amnt, _house_amnt) = calcBetShort(msg.value, _rollAmnt, _isUnder);
        ...
        if(clean) {
            uint _lengthDecrease = 0;
            for(uint c=0; c<rollToComplete.length; c++) {
                if(rollToComplete[c].finalized==true) {
                    if(c+1 < rollToComplete.length) {
            ...
            if(_lengthDecrease >0) {
                if(rollToComplete.length <= _lengthDecrease )
                    rollToComplete.length = 0;
                else
                    rollToComplete.length = rollToComplete.length - _lengthDecrease;
            }
        }
        ...
        tronwinVault_I.receiveExternalGameFund.value(msg.value)(false);
    ...
    mapping(address => uint) public addressPositionInRollToComplete;


    function finalizePlayer(uint _rollToClompletePos, address _player, uint _result) public onlyOwner {        
        //PendingRoll storage _pendingRoll = pendingRolls[_player];

        require(pendingRolls[_player].isFinalized == false);
        ...
        //        calcBet(pendingRolls[_player].rollValue, pendingRolls[_player].rollAmnt, pendingRolls[_player].isUnder);

        uint _rollAmnt = pendingRolls[_player].rollAmnt;
        bool _isUnder = pendingRolls[_player].isUnder;


        if(  (_isUnder == true && _result < _rollAmnt) 
                    || (_isUnder == false && _result > _rollAmnt)  ) {
            
            //tronwinVault_I.receiveExternalGameFund.value(pendingRolls[_player].rollValue)(false); - do in roll
            //tronwinVault_I.receiveExternalGameFund.value(pendingRolls[_player].rollValue)(true);
            tronwinVault_I.applyFundsAsTokenEligible(pendingRolls[_player].rollValue);
            tronwinVault_I.gamingFundPayment(_player, pendingRolls[_player].pending_winAmnt, true);
            // result left in pot = house_amnt

            totalWon = totalWon + pendingRolls[_player].pending_winAmnt;
            playerWinners[_player] = playerWinners[_player] + pendingRolls[_player].pending_winAmnt;

            if(pendingRolls[_player].pending_winAmnt > topWin) {
                topWin = pendingRolls[_player].pending_winAmnt;
                topPlayer = _player;
            }
            ...
            emitRollResult(0, _player, _result, pendingRolls[_player].pending_winAmnt);
            ...
            uint _lossMaintainVal = m_lossMaintainPercent.mul(pendingRolls[_player].rollValue);


            //tronwinVault_I.receiveExternalGameFund.value(_lossMaintainVal)(false);
            // remainder is profit for token holders!
            //tronwinVault_I.receiveExternalGameFund.value( pendingRolls[_player].rollValue - _lossMaintainVal )(true);
            tronwinVault_I.applyFundsAsTokenEligible(pendingRolls[_player].rollValue - _lossMaintainVal);

            playerLosses[_player] = playerLosses[_player] + pendingRolls[_player].rollValue;
            totalLosses = totalLosses + pendingRolls[_player].rollValue;
        ...
        */
        ...
        rollToComplete[_rollToClompletePos].finalized = true;
    ...
    function p_setCleanOn() public onlyOwner {
        clean = true;
    }
    function p_setCleanOff() public onlyOwner {
        clean = false;
    }
