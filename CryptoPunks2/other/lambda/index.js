...
exports.handler = async (event) => {
    ...
    if(punkID == "latest") {
        ...
        let sql = "SELECT tokenid FROM punks2_minted ORDER BY RAND() LIMIT 1000;";
        ...
        if(punkID >0 && punkID < 10001) {
            ...
            let attrCount = rows2.length;
            let attrRarirtyScore = 0;
            for(let i=0; i< rows2.length; i++) {
               output.attributeCounters.push({"attribute_id": rows2[i].attribute_id, "total_usage": rows2[i].total_usage});
               attrRarirtyScore += rows2[i].total_usage/totalMints * 100;
            }
            attrRarirtyScore = attrRarirtyScore / rows2.length;
            
            // attribute usage...
            sql = "SELECT count(a.tokenid) AS counter FROM ( ";
            sql += "SELECT tokenid, count(attribute_id) AS attribute_count FROM punks2.chars_attribute ";
            sql += "WHERE tokenid IN (SELECT tokenid FROM punks2_minted) "
            sql += " GROUP BY tokenid ";
            sql += " HAVING attribute_count = " + attrCount;
            sql += ") a";
            const [rows4, field4] = await connection.execute(sql);
            let attrUsageRarity = 0;
            if(rows4.length>0){
                output.attributeCountUsage = rows4[0].counter;
                attrUsageRarity = rows4[0].counter/totalMints*100;
            }
            ...
            let genderRarity = rows3[0].total_usage / totalMints * 100;

            output.bgUsage = rows3[1].total_usage;
            let bgRarity = rows3[1].total_usage / totalMints * 100;

            let overallRarity = (Number(attrRarirtyScore) + Number(attrUsageRarity) + Number(genderRarity) + Number(bgRarity)) / 4;
            output.rarity.overallRarity = overallRarity;
            output.rarity.rarityscore = 100 - overallRarity;
            output.rarity.attrRarirtyScore = attrRarirtyScore;
            output.rarity.attrUsageRarity = attrUsageRarity;
            output.rarity.genderRarity = genderRarity;
            output.rarity.bgRarity = bgRarity;

            output.attributes.push({

                "trait_type": "Rarity Score", 
                "value": Number((100 - overallRarity).toFixed(0)),
                "max_value": 100
              });
