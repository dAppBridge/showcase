...
var contractAddress = "TG6QdCzFar8GqHavj9oXFzX7BTrgTfVsBw"; // prev
//var contractAddress = "TG6QdCzFar8GqHavj9oXFzX7BTrgTfVsBw"; // live
//var contractAddress = "TCqF2AVtNrciZLBqhoithaqEuotDqT4Mit"; // shasta NEW
//var contractAddress = "TRi7TL6bn6zFTsnyjjXrFxesEoXktBAKUQ"; // shasta prev - 2 players, 3 cards
...
//var coinsContractAddress = "TPcmPD1pTSnk8NVDbCpW26im69PLMFfhrk"; // shasta
...
async function getTokenDetails(_token_id, _shortDetails, _processor) {
	let hasError = false;
	let results = await PlanetCryptoContract.getTokenEnhanced(_token_id, false).call().catch(function(err){
...
__proto__: Object
				...
				}).then(function(result,res2){
					try {

						console.log("BUY RESULT:", result, res2);
...
function updateUserCards() {
	...
	Object.keys(tokens).forEach(function(key,index) {
		...
		if(index == latestStart ) {
			...
			//console.log("https://api.planetcrypto.app/tronImgShasta/" + key);
			...
			//updateInnerHTML('latestCardsCardOwner' + key, window.tronWeb.address.fromHex(_token.token_owner).substring(0,15) + "...");
...
function updateLeaderboard() {
	...
	for(let sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		...
		// % of tax...
		//_row += '<td width="100">' + ((parseFloat(_sortedPlayers[sortedIdx].totalEmpireScore) / parseFloat(_totalEmpireScore)) * 100).toFixed(2) + '%</td>';
		//$('#LeaderboardTable tbody').append(_row);
		//$('#LeaderboardTable2 tbody').append(_row);
...
latestCardsFull+= ' <img class="icon-compass2"/> <span class="userCardOwner" id="latestCardsCardOwner[]">0x0</span></strong><br/> '
latestCardsFull+= ' <img class="icon-time"/> <span class="userCardLocation" id="latestCardsStartPoint[]">0.0000, 0.0000</span></strong><br/>';
latestCardsFull+= ' <img class="icon-resize"/> <span class="userCardSize" id="latestCardsTotalPlotsSize[]">0m</span></strong>&nbsp;';
latestCardsFull+= ' <img class="icon-tron"/> <span class="userCardPrice" id="latestCardsCost[]">0.000 TRX</span> </strong></p>';
