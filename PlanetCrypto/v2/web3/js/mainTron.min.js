...
var playing = false;
var canFlyHome = true; //var contractAddress = "TM7xwW5F4HideRk61v5XFDE4SBe77TFQpA"; //shasta //"TTFnbwbySrENg9aGYUkNYos12YdB1Q6huB"; // v2
//var contractAddress = "TQedJmWPJzrJfGTvfiKdNY7ZW5C8xsYgAt"; // live

var contractAddress = "TPTCVbwSxmNS1nDbcbaz18v9mMk2DejGtG"; //live NEW - testing

var contractAddressOld = "TQedJmWPJzrJfGTvfiKdNY7ZW5C8xsYgAt"; //live NEW - testing
...
var CONTRACT_START_TIME = 1548174345000; //var CONTRACT_START_TIME = 1548174345;
...
var rectsToBuy = {}; //var currentDisplayMinGridMinX = 180;
...
var isMobile = false;
var shareEarnEthBtn;
var provider; //isMobile = true; // DEBUG DEBUG DEBUG
//window.addEventListener('load', async () => {

var isReadonly = true;
var currentSearchDisplayPage = 1;
var searchMaxPages = 1; // ENTRY
...
window.addEventListener('load', function () {
  ...
  function setupTron() {
    if (window.tronWeb) {
      ...
      */
      ...
      var tronWeb = new TronWeb(fullNode, solidityNode, eventServer, NULL_ADDR);
      window.tronWeb = tronWeb;
    }

    if (window.location.href.indexOf("card.htm") > -1) isCard = "1";
    console.log("Init Referer:", _referer);
    console.log("Hex Fix:", solidityEncodeAddr(_referer)); //_referer = NULL_ADDR;
  ...
  if (isCard == "1") {
    ...
    if (isMainGame) setupMap();else {
      var _searchAddress = getUrlParameter('ownerAddress') || "";
    ...
    if (_newReferer.length > 0) {
      // newly referred
      _referer = _newReferer;
      Cookies.set('tron', _referer, {
        expires: 30,
        path: '/'
      });
    } else {
      // existing user referred by someone
    ...
    $('#buyUpsellButtonConfirm').click(function () {
      buyWithTrx();
    });
    $("#buywthTRX").click(function () {
      buyWithTrxUpsell();
    });
    $("#buywthTokens").click(function () {
      buyWithTokens();
    });
    $("#withdrawButton").click(function () {
      withdrawFunds();
    });
    $("#withdrawButton2").click(function () {
      withdrawFunds();
    ...
    $("#cryptoGuideBtn").click(function () {
      showGuide();
    });
    $("#cryptoGuideBtnMobile").click(function () {
      showGuide();
    });
    $("#buyMultiple").click(function () {
      startBuyMultiple();
    });
    $("#buySingle").click(function () {
      startBuySingle();
    });
    ...
    $("#buySingleMobile").click(function () {
      startBuySingle();
    });
    $("#removeSingleMobile").click(function () {
      startRemoveSingle();
    });
    ...
    $("#newCardStartPointLink").click(function () {
      gotoStartPointLink();
    });
    $("#existingCardStartPointLink").click(function () {
      gotoExistingCardStartPoint();
    });
    ...
    $('#buyExistingWthETH').click(function () {
      buyExistingCard();
    });
    $('#destoryButtonBuy').click(function () {
      buyExistingCard();
    });
    $('#destoryButtonConfirm').click(function () {
      destroyCard();
    });
...
async function getTokenDetails(_token_id, _shortDetails, _processor) {
  var hasError = false;
  var results = await PlanetCryptoContract.getToken(_token_id, _shortDetails).call().catch(function (err) {
...
function processLogResult(logResult, isInit, isFinalItem, callback_func) {
  ...
  */
  ...
  if (logResult.name == 'Transfer') {
    if (logResult.result.from == NULL_ADDR) {// is creation = ignore
    } else {
      if (tokens[logResult.result.tokenId]) {
        ...
        if (isInit) {} else {
          ...
          if (logResult.result.to.toUpperCase() == userWalletAddress.toUpperCase()) {
            ...
            tokens[logResult.result.tokenId].token_owner = logResult.result.to; // add to previous cards (With updated details)

            processPreviousCardsToken(tokens[logResult.result.tokenId], logResult.result.tokenId); // update details on latestCardFull

            $('#latestCardHeader' + logResult.result.tokenId).css("background-color", '#' + logResult.result.to.substring(logResult.result.to.length - 6));
            updateInnerHTML('latestCardsCardOwner' + logResult.result.tokenId, logResult.result.to.substring(0, 20) + "..."); // add to your cards...
          ...
          if (logResult.result.from.toUpperCase() == userWalletAddress.toUpperCase()) {
            ...
            if (logResult.result.to == NULL_ADDR) {
              ...
              $('#latestCardFull' + logResult.result.tokenId).remove(); // and remove from the map... 

              delete tokens[logResult.result.tokenId];
              moveEndEvent(mymap.getZoom());
              queryMap(mymap.getZoom());
            } else {
              console.log("Transfer changing details"); // change details on token[] object
      ...
      } else {//console.log("DEBUG -> TOKEN NOT FOUND", logResult.result.tokenId);
      }
    }
  ...
  async function setupContracts() {
    ...
    */
    ...
    try {
      ...
      _msg += '<p style="font-size: 10pt;">Please refresh the page to try again.</p>';
    ...
    if (contractSetupHasError) return;
    ...
    if (!isReadonly) {
      ...
      if (1 == 1) {
        ...
        var _account = await window.tronWeb.trx.getAccount().catch(function (err) {
          ...
          if (!hasError) {
            ...
            userWalletAddressShort = userWalletAddress.substring(0, 15) + "...";
            setNewCardBg(); // https://www.cssscript.com/create-social-share-popup-box-with-pure-javascript-needsharebutton/
...
var _maxPages = 10;
var _tokens = [];
var _startTime = CONTRACT_START_TIME; //CONTRACT_START_TIME
//1549533801000
//1548174345000
...
function getLandPurchasedEvent(_currentPage) {
  async function getEvents(_currentPage) {
    ...
    */
    ...
    window.tronWeb.getEventResult(contractAddress, {
      ...
      for (var c = 0; c < logResults.length; c++) {
        ...
        if (c == lastCardSaleID) {
          ...
          else {//processLogResult(logResults[c], true, true);
            }
        } else {//processLogResult(logResults[c], true, false);
          }
      }
    }).then(function () {
      if (_currentPage + 1 < _maxPages) getLandPurchasedEvent(_currentPage + 1);else {
        console.log("TOKENS LEN:", _tokens.length); //    grabTokens();
      }
    });
  }
  ...
  async function grabTokens() {
    for (var c = 0; c < _tokens.length; c++) {
...
var lastHighestTokenId = 0;
var lastTokenIDCount = 0;
var _tokenIDCount = 0;
var _initTotalReceived = 0;
var _maxBatch = 300;
var _batchDelay = 500;
  ...
  PlanetCryptoContract.tokenIDCount().call().then(function (result) {
    ...
    if (_tokenIDCount > lastTokenIDCount) {
      var _ret = function () {
        ...
        var _markersToAdd = []; // do this in batches of 20 gets...
        ...
        var _loop = function _loop(c) {
          ...
          if (tokens[_token_id]) {
            if (_token_id == _tokenIDCount) {
              ...
              processTransfers();
              updateUserCards(); //mymap.addLayer(markerCluster);
            }
          } else {
            getTokenDetails(_token_id, false, function (results) {
              if (isInit) _initTotalReceived++;
              ...
              if (results) {
                ...
                updateInnerHTML('totalCards', totalCards);
                updateInnerHTML('totalCardsMobile', totalCards);
                if (_token_id > lastHighestTokenId) lastHighestTokenId = _token_id;
                ...
                if (isMainGame) {
                  if (fireGameDataReadyCounter == 0 || c == _tokenIDCount - 1) {
                    ...
                    */
                    ...
                    _markersToAdd = [];
                    if (isMobile) fireGameDataReadyCounter = 100;else fireGameDataReadyCounter = 50; //startFireGameDataReady();
                    //processTransfers();
                  } else {
                    fireGameDataReadyCounter--;
                  }

                  if (markersAllLvls[mymap.getZoom() + ':' + _token_id]) {//console.log("Already set");
                  } else {
                    ...
                    var _marker = createMarker(_token_id, tokens[_token_id], latLngPoint); //_marker.addTo(mymap);
              ...
              if (_initTotalReceived == _tokenIDCount - 1) {
                console.log("Processed last token at setup");
                $("#loadingDiv").addClass('hide'); //startFireGameDataReady();
        ...
        for (var c = lastTokenIDCount + 1; c < _tokenIDCount; c++) {
          var _ret2 = _loop(c);
...
function processInitToken(results, _token_id) {
  ...
  if (results) {
    ...
    updateInnerHTML('totalCardsMobile', totalCards);
    if (_token_id > lastHighestTokenId) lastHighestTokenId = _token_id;
    if (_token_id > lastTokenIDCount) lastTokenIDCount = _token_id;
    ...
    if (fireGameDataReadyCounter == 0 || _token_id == _tokenIDCount - 1) {
      fireGameDataReadyCounter = 20; //startFireGameDataReady();
      //processTransfers();
    } else {
      fireGameDataReadyCounter--;
    }
  ...
  if (_token_id == _tokenIDCount) {
    ...
    $("#loadingDiv").addClass('hide');
    startFireGameDataReady();
    processTransfers(); //mymap.addLayer(markerCluster);
  } else {
    // do more...
...
function setupLogSubscription() {
  ...
  if (finalLogItemProcessed == false) {
    ...
    */
    ...
    checkForNewTokens(true); //getLandPurchasedEvent(1);
    // now grab the tokens...
  }
}
...
function processTransfers() {
  ...
  for (var page = 1; page < _maxPages + 1; page++) {
    window.tronWeb.getEventResult(contractAddressOld, CONTRACT_START_TIME, "cardBought", false, 200, page).then(function (logResults) {
      if (logResults.length == 0) {
        updatePreviousCards();
      } else {
        var lastCardSaleID = logResults.length - 1;
        ...
        for (var c = 0; c < logResults.length; c++) {
          if (c == lastCardSaleID) {
            if (page == _maxPages) processLogResult(logResults[c], true, true, updatePreviousCards);else processLogResult(logResults[c], true, true);
          } else {
            processLogResult(logResults[c], true, false);
          }
        }
  ...
  for (var page = 1; page < _maxPages + 1; page++) {
    window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, page).then(function (logResults) {
      //console.log("Tron Bought logs:", logResults);
      if (logResults.length == 0) {
        updatePreviousCards();
      } else {
        var lastCardSaleID = logResults.length - 1; //console.log("logResults INIT", logResults);
...
function processTransfers2() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 2).then(function (logResults) {
    console.log("processTransfers2"); //console.log("Tron Transfer logs:", logResults);
...
function processTransfers3() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 3).then(function (logResults) {
    console.log("processTransfers3"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers4);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers4() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 4).then(function (logResults) {
    console.log("processTransfers4"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers5);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers5() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 5).then(function (logResults) {
    console.log("processTransfers5"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers6);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers6() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 6).then(function (logResults) {
    console.log("processTransfers6"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers7);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers7() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 7).then(function (logResults) {
    console.log("processTransfers7"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers8);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers8() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 8).then(function (logResults) {
    console.log("processTransfers8"); //console.log("Tron Transfer logs:", logResults);
    ...
    if (logResults.length == 0) {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers9);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers9() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 9).then(function (logResults) {
    //console.log("processTransfers9");
    //console.log("Tron Transfer logs:", logResults);
    if (logResults.length == 0) {
      updatePreviousCards();
    } else {
      ...
      for (var c = 0; c < logResults.length; c++) {
        if (c == lastCardSaleID) {
          processLogResult(logResults[c], true, true, processTransfers10);
        } else {
          processLogResult(logResults[c], true, false);
        }
      }
...
function processTransfers10() {
  window.tronWeb.getEventResult(contractAddress, CONTRACT_START_TIME, "cardBought", false, 200, 10).then(function (logResults) {
    console.log("processTransfers10"); //console.log("Tron Transfer logs:", logResults);
...
function initStats(_callback) {
  updateCurrentLandPrice(); // TO TEST TRON WITH PLAYERS
  ...
  async function getCurrentPlayers() {
    ...
    var currentPlayersLen = await PlanetCryptoContract.getAllPlayerObjectLen().call();
    ...
    for (var c = 1; c < currentPlayers; c++) {
      var _playerObj = await PlanetCryptoContract.all_playerObjects(c).call();
...
function updateMainGameStats(withPlayerCount, _callback) {
  if (withPlayerCount == true) {
    var getCurrentPlayers = async function getCurrentPlayers() {
      //let val = await PlanetCryptoContract.current_plot_price().call();
      var currentPlayersLen = await PlanetCryptoContract.getAllPlayerObjectLen().call();
      currentPlayers = currentPlayersLen.toNumber();
      if (currentPlayers == 0) currentPlayers = 1;
  ...
  async function getBaseStats() {
    var _totalTrades = await PlanetCryptoContract.total_trades().call();

    totalTrades = _totalTrades.toNumber();
    updateInnerHTML('totalTrades', totalTrades);
    updateInnerHTML('totalTradesMobile', totalTrades);

    var _taxFund = await PlanetCryptoContract.tax_fund().call();
    ...
    var _taxFund2 = await PlanetCryptoContractOld.tax_fund().call();
    ...
    var _taxDist = await PlanetCryptoContract.tax_distributed().call();
    ...
    var _taxDist2 = await PlanetCryptoContractOld.tax_distributed().call();
  ...
  getBaseStats(); // now the players PlanetCOINs balance...

  if (!isReadonly) updatePlayerPlanetCOINs();else {
    yourCOINS = 0;
    updateInnerHTML('yourCOINS', yourCOINS);
    updateInnerHTML('yourCOINSMobile', yourCOINS);
  } // and tax earnings available...

  if (!isReadonly) updateTaxEarningsAvail();else {
    tax_available = 0;
    tax_available2 = 0;
...
function updateTaxEarningsAvail() {
  async function updateTaxAvail() {
    var _taxAvail = await PlanetCryptoContract.taxEarningsAvailable().call();

    tax_available = new BigNumber(_taxAvail.toNumber());

    var _taxAvail2 = await PlanetCryptoContractOld.taxEarningsAvailable().call();

    tax_available2 = new BigNumber(_taxAvail2.toNumber());
    updateInnerHTML('yourTaxAvailable', window.tronWeb.fromSun(tax_available.plus(tax_available2).toString()).substring(0, 8));
    updateInnerHTML('yourTaxAvailableMobile', window.tronWeb.fromSun(tax_available.plus(tax_available2).toString()).substring(0, 8));
  }
...
function updatePlayerPlanetCOINs() {
  async function updatePlanetCOINs() {
    var _coins = await PlanetCryptoCoinContract.balanceOf(userWalletAddress).call();

    yourCOINS = _coins.toNumber();
    updateInnerHTML('yourCOINS', yourCOINS);
    updateInnerHTML('yourCOINSMobile', yourCOINS);

    var _allowance = await PlanetCryptoCoinContract.allowance(userWalletAddress, contractAddress).call();

    var _newAllowance = _allowance.toNumber();
...
function updateCurrentLandPrice(_callback) {
  async function doGetCurrentLandPrice() {
    var hasError = false;
    var val = await PlanetCryptoContract.current_plot_price().call().catch(function (err) {
      ...
      if (!hasError) {
        ...
        if (isMainGame) updateInnerHTML('currentLandPrice', window.tronWeb.fromSun(currentLandPrice.toString()).substring(0, 8));
      }
    });
    /*.catch(function(err){
      var _msg = ""
...
function withdrawFunds() {
  ...
  async function doWithdrawel(_contract, _callback) {
    ...
    var result = _contract.withdrawTaxEarning().send({
      shouldPollResponse: true
    }).catch(function (err) {
      hasError = true; //Not enough energy for 'ISZERO' operation executing…100000000], curOpEnergy[3], usedEnergy[100000000]"
      ...
      if (!hasError) {
        var _msg = ""; //_msg += '<p style="font-size: 10pt;"><i>Withdrawel of Tax Earnings Complete!</i></p>';
        ...
        showError(_msg, "Withdrawal Complete!"); //tax_available = new BigNumber(0);
        //updateInnerHTML('yourTaxAvailable', "0");
        //updateInnerHTML('yourTaxAvailableMobile', "0");
  ...
  if (tax_available2.gt(0)) doWithdrawel(PlanetCryptoContractOld, function () {
    updateTaxEarningsAvail();
  });else doWithdrawel(PlanetCryptoContract, function () {
    updateTaxEarningsAvail();
  });
}
...
function destoryCardProcess() {
  ...
  async function doDestroy(_existingCardTokenId) {
    ...
    var hasError = false;
    var result = await PlanetCryptoContract.burn(_existingCardTokenId).send().catch(function (err) {
      ...
      if (!hasError) {
        var _msg = "";
        _msg += '<p style="font-size: 10pt;"><i>Your new PlaneteCOINs will be in your wallet now.</i></p>';
...
function buyExistingCardProcess() {
  ...
  async function doBuyCard(_existingCardTokenId) {
    ...
    var result = PlanetCryptoContract.buyCard(_existingCardTokenId, _referer).send({
  ...
  if (_referer == NULL_ADDR) {
    ...
    if (window.tronWeb.isAddress(_referer)) {
      console.log("Referer:", _referer); //buyLandFnc(strToBytes32Array(_encodedName), _lat, _lng, _referer, playAmountSun);

      doBuyCard(existingCardTokenId, _referer);
    } else {
      console.log("Invalid Referer"); //buyLandFnc(strToBytes32Array(_encodedName), _lat, _lng, NULL_ADDR, playAmountSun);  
...
function buyWithTrxProcess() {
  ...
  updateCurrentLandPrice(function () {
    ...
    */
    ...
    var _name = strToBytes32Array(_encodedName);

    var _amnt = playAmountSun;
    ...
    buyLandFnc(); //buyLandFnc(strToBytes32Array(_encodedName), _lat, _lng, NULL_ADDR, playAmountSun);
...
function buyWithTokenProcess() {
  ...
  async function doBuyLandWithTokens() {
    ...
    var result = await PlanetCryptoContract.buyLandWithTokens(_encodedName, _lat, _lng).send({
      shouldPollResponse: true
    }).catch(function (err) {
      //TODO - need to show this error to the user!!
      //Not enough energy for 'ISZERO' operation executing…100000000], curOpEnergy[3], usedEnergy[100000000]"
      ...
      if (!hasError) {
        ...
        showError(_msg, "Purchase Complete");
        resetSelectedRects();
        ga('send', 'event', 'BuyTools', 'BuyWithTokensComplete'); //gtag_report_conversion();
      }
    });
  }
...
function switchCOINlockstatus() {
  ...
  async function doUnlock() {
    ...
    var hasError = false;
    var results = PlanetCryptoCoinContract.approve(contractAddress, _amount).send({
      shouldPollResponse: true
    }).catch(function (err) {
      hasError = true; //TODO - need to show this error to the user!!
      //Not enough energy for 'ISZERO' operation executing…100000000], curOpEnergy[3], usedEnergy[100000000]"
      // Note: once energy has been gained the wallet/page needs refreshing
      ...
      if (!hasError) {
        ...
        _msg += '<p style="font-size: 10pt;">';
        _msg += ' For every new player you refer on - you will earn 5% from purchases as an instant bonus!';
        _msg += '</p>'; //_msg += '<div class="addthis_inline_share_toolbox"></div>';
...
function queryMap(zoomLvl) {
  ...
  var _remover = 2; //, grid17.currentRows, grid17.currentColumns
  ...
  if (zoomLvl == 18) {
    ...
    _remover = 0;
    zoom = 0; // lat = grid18.currentColumns.currentRows
  ...
  if (zoomLvl == 16) {
    // generate queryMap(uint8 zoom, int256[] lat_rows, int256[] lng_columns)
    zoom = 4;
    _divisor = 10000; //_latRows = grid16.currentRows;
  ...
  if (zoomLvl == 15) {
    zoom = 3;
    _divisor = 1000;
    _remover = 3; //_latRows = grid15.currentRows;
    //_lngColumns = grid15.currentColumns;
  }

  if (zoomLvl < 15) {//   return;
  }

  if (zoomLvl == 14) {
    zoom = 3;
    _divisor = 1000;
    _remover = 3; //_latRows = grid14.currentRows;
  ...
  */
  ...
  var latlongBound = mymap.getBounds(); //console.log(tokens);
  ...
  if (zoom == 0) {
    ...
    Object.keys(tokens).forEach(function (key, index) {
      ...
      if (!latlongBound.contains(latLngPoint)) {//console.log("Not in view:", key); // ignore this point
        ...
        if (zoom == 0) {
          ...
          var _token_id = key; //var _token = tokens[_token_id];

          var _token = tokens[key];

          for (var c = 0; c < _token.card_size; c++) {
            var _latLngPoint = L.latLng([parseFloat(hardFixed(_token.plots_lat[c] / 1000000, 5)), parseFloat(hardFixed(_token.plots_lng[c] / 1000000, 5))]);
          ...
          // show markers direct
          var _token_id = key;
          var _token = tokens[key]; //console.log("token_id", _token_id);

          if (markersAllLvls[zoomLvl + ':' + _token_id]) {//console.log("Already set");
            ...
            var _marker = createMarker(_token_id, _token, latLngPoint); //mainMarkersLayer.addLayer(_marker);
    ...
    Object.keys(lightGrids[zoomLvl].cellDetails).forEach(function (cellKey, cellIndex) {
      ...
      for (var c = 0; c < _secondaryLookups.length; c++) {
        if (lightGrids[zoomLvl].cellDetails[cellKey].bounds.contains(_secondaryLookups[c].latLngPoint)) {
          ...
          var _rect = L.rectangle(lightGrids[zoomLvl].cellDetails[cellKey].bounds, {
            color: '#' + _secondaryLookups[c].token.token_owner.substring(_secondaryLookups[c].token.token_owner.length - 6),
            weight: 2,
            fillOpacity: 0.5
          });

          lightGrids[zoomLvl].rects[lightGrids[zoomLvl].cellDetails[cellKey].key].token_id = _secondaryLookups[c].token_id;
          /*
          ...
          */
          ...
          _msg += "<br/>~ You can run a hostile take-over and own this card for: " + window.tronWeb.fromSun((_secondaryLookups[c].token.current_value * CURRENT_RESALE_MULTIPLIER).toString()).substring(0, 8);
          _msg += " TRX.";
          _msg += "<br/>~ Doing so will also increase it's EMPIRE SCORE to: " + _secondaryLookups[c].token.empire_score * CURRENT_EMPIRE_MULTIPLIER;
          _msg += "<br/> &nbsp;&nbsp; - increasing your score whilst reducing the previous owners!";
          _msg += "<br/><br/><center><span class='smallTxt'><strong>Click for full card details</strong></span></center>"; //lightGrids[zoomLvl].rects[rectsKey].bindTooltip(_msg);

          _rect.bindTooltip(_msg);

          try {//lightGrids[zoomLvl].rects[rectsKey].removeFrom(mymap);
          } catch (e) {} //lightGrids[zoomLvl].rects[rectsKey].addTo(mymap);
          ...
          _rect.addTo(mymap); //console.log(_rect);
...
function createMarker(_token_id, _token, latLngPoint) {
  ...
  var _marker = L.marker(latLngPoint, {
    icon: _Marker
  });

  var _msg = "<span style='font-size:0.9em;'><b style='font-size:1.2em;'>Token: #" + _token_id + " ~ " + _token.name + "</b><br/>";

  _msg += "<b>Owner:</b><br/> " + window.tronWeb.address.fromHex(_token.token_owner) + ""; //_msg += "<br/><b>Total Size:</b> " + _token.card_size * PLOT_SIZE + "m<sup>2</sup>";
  ...
  _msg += "<br/>~ Run a hostile take-over for: <strong>" + window.tronWeb.fromSun((_token.current_value * CURRENT_RESALE_MULTIPLIER).toString()).substring(0, 8);
  _msg += " TRX</strong>";
  _msg += "<br/>~ Increasing it's EMPIRE SCORE to: <strong>" + _token.empire_score * CURRENT_EMPIRE_MULTIPLIER + '</strong>'; //_msg += "<br/> &nbsp;&nbsp; - increasing your score whilst reducing the previous owners!"

  _msg += "<br/><center><strong>~ Click for Full Card Details ~</strong></center></span>";

  _marker.bindTooltip(_msg);
...
var markersAllLvls = {}; // TODO

function manageCaches() {// clear out old cache items on timeout
}
...
function clearGrids() {//for(key in lightGrids) {
...
function clearDisplayedKeys() {
  ...
  rectsDisplayedKeys = []; //console.log("AFTER", rectsDisplayedKeys);
}
...
function resetSelectedRects() {
  ...
  actionMode = '';
  resetButtons();
  if (allowedPlotsCircle) allowedPlotsCircle.remove();
  clearNewBuyCard();
  updateNewCard();
}
...
function showExistingCard(_token_id, _flyTo) {
  ...
  if (tokens[_token_id]) {} else {
    console.log("Token not loaded..."); // get it now...
    ...
    */
    ...
    setTimeout(function () {
      showExistingCard(_token_id, _flyTo);
    }, 1000);
  }

  var _token = tokens[_token_id];
  if (!_token) return; //console.log(_token);

  _findTokenId = "";
  $('#existingCardHeader').css("background-color", '#' + _token.token_owner.substring(_token.token_owner.length - 6));
  if (isMobile) updateInnerHTML('existingCardOwner', window.tronWeb.address.fromHex(_token.token_owner).substring(0, 10) + "...");else updateInnerHTML('existingCardOwner', window.tronWeb.address.fromHex(_token.token_owner).substring(0, 25) + "...");
  if (_token.token_owner.toUpperCase() == userWalletAddress.toUpperCase()) existingCardOwned = true;else existingCardOwned = false;
  document.getElementById('existingCardOwner').value = _token.token_owner;
  if (_token.plots_lng.length > 1) updateInnerHTML('existingCardTotalPlots', _token.plots_lng.length + " Plots");else updateInnerHTML('existingCardTotalPlots', _token.plots_lng.length + " Plot");
  ...
  updateInnerHTML('existingCardTotalPlotsSize', _LandSize + _LandSizeUnit); //updateInnerHTML('existingCardsEmpireScore', _token.empire_score);
  ...
  */
  ...
  $('#existingCardImgBack2').css("background-image", "url(https://api.planetcrypto.app/tronImg/" + _token_id + ")"); //$('#existingCardImg').attr("src","img_srv/index.htm?lat=" + _token.plots_lat[0]/1000000 + "&lng=" + _token.plots_lng[0]/1000000);
  ...
  $("meta[property='og:image']").attr("content", 'https://api.planetcrypto.app/tronImg/' + _token_id); //$('#existingCardDiv').removeClass('hide');
...
function gotoExistingCardStartPoint() {
  try {
    if (mymap.getZoom() < MIN_BUY_LVL) {
      mymap.flyTo(existingCardStartPoint, 18);
    } else {
      mymap.flyTo(existingCardStartPoint, 18);
    }
  } catch (e) {}
...
function setupMap() {
  if (!isMobile) {
    ...
    mymap = L.map('mapid', {
      ...
      boxZoom: false,
      scrollWheelZoom: true
    }).setView([38.3796072, -93.0596094], 4, {
      paddingTopLeft: [300, 300]
    });
  ...
  mymap.setMaxZoom(18);
  mymap.setMinZoom(3); //setupImgMap();
  ...
  if (!isMobile) {
    markerCluster.on('clustermouseover', function (c) {
      var popup = L.popup().setLatLng(c.layer.getLatLng()).setContent(c.layer._childCount + ' Cards in area (Click to Zoom)').openOn(mymap);
  ...
  function startWithUserPos(location) {
    ...
    if (_findTokenId.length > 0) {} else {
      locateMap(new L.LatLng(location.coords.latitude, location.coords.longitude));
    }
  }

  function startWithDefaultPos(error) {//console.log("startWithDefaultPos", error);
  }

  function locateMap(locate_latlng) {
    console.log("Flying to:", locate_latlng);
    if (canFlyHome) mymap.flyTo(locate_latlng);
  }
  ...
  mymap.on('zoomstart', function (ev) {
    ...
    clearMarkers();
    clearGrids(); // clear up rectsDisplayedKeys
  ...
  mymap.on('zoomend', function (ev) {
    ...
    if (grid14Iinit == false && ev.target._zoom == 14) {
      if (rectsToBuyLen() > 0) {
        currentlySelectedMarker = L.marker(rectsToBuy[Object.keys(rectsToBuy)[0]], {
    ...
    if (ev.target._zoom > 16) {
      if (!lightGrids[ev.target._zoom]) {
        ...
        console.log("READY"); //queryMap(ev.target._zoom);
      }
    } else {}
    ...
    if (ev.target._zoom > 16) {//lightGrids[ev.target._zoom].addTo(mymap);
    }

    if (ev.target._zoom > 12) {//lightGrids[ev.target._zoom].addTo(mymap);  
    }
    ...
    if (ev.target._zoom < 15) {
      ...
      if (rectsToBuyLen() > 0) allowedPlotsCircle.addTo(mymap);
...
function startRemoveSingle() {
  if (mymap.getZoom() < MIN_BUY_LVL) mymap.setZoom(MIN_BUY_LVL);
...
function gotoStartPointLink() {
  try {
    if (mymap.getZoom() < MIN_BUY_LVL) {
      mymap.flyTo(L.latLng(document.getElementById('newCardStartPoint').innerHTML.split(',')), 18);
    } else {
      mymap.flyTo(L.latLng(document.getElementById('newCardStartPoint').innerHTML.split(',')), 18);
    }
  } catch (e) {}
...
function updateNewCard() {
  ...
  if (_rectsToBuyLen > 0) {
    updateInnerHTML('newCardStartPoint', rectsToBuy[Object.keys(rectsToBuy)[0]].lat.toFixed(4) + "," + rectsToBuy[Object.keys(rectsToBuy)[0]].lng.toFixed(4));
    ...
    // reset card to emtpy..
    clearNewBuyCard(); // clear out the circle too..

    try {
      allowedPlotsCircle.remove();
    } catch (e) {} //delete allowedPlotsCircle;
...
function checkforFirstPlot(_rectsToBuyLen) {
  if (_rectsToBuyLen == 1) {
    showNewCard();
    if (allowedPlotsCircle) allowedPlotsCircle.remove(); // is first plot so add in the max circumfrance layer...
...
function showError(error, errorTitle) {
  ...
  } else {//updateInnerHTML('errorHeader', 'PlanetCrypto');
  }
...
function hardFixed(_in, _decimals) {
  ...
  if (_a.length > 0) {
    return parseFloat(_a[0].toString() + "." + _a[1].toString().substring(0, _decimals));
  } else {
...
function normaliseLatLngUp(_in) {
  ...
  if (_rightDigit <= 5) {
    ...
    _rightDigit2++; //console.log("normaliseLatLngUp2:", parseFloat(_in.toString().substring(0,_len-2) + _rightDigit2.toString() + '1'));

    return parseFloat(_in.toString().substring(0, _len - 2) + _rightDigit2.toString() + '1');
  }

  return _in; //M {lat: 51.5085, lng: -0.0972}  
}
...
function normaliseLatLngDown(_in) {
  ...
  if (_rightDigit >= 5) {
    _rightDigit2++; //console.log("normaliseLatLngDown", parseFloat(_in.toString().substring(0,_len-1) + _rightDigit2.toString() + '1'));
...
function updatePreviousCards() {
  $("#userLostCards").empty();
  $("#userLostCards2").empty(); //console.log("updatePreviousCards", tokens);
...
function processPreviousCardsToken(_token, key) {
  ...
  if (_token.token_owner.toUpperCase() != userWalletAddress.toUpperCase()) {
    ...
    if (usedToOwn) {
      ...
      if (existingDisplay) {} else {
        var _newCard = latestCardsFull.replace(/\[\]/g, '_previous_' + key);

        if (!isMobile) $("#userLostCards").prepend(_newCard);else {
      ...
      if (_token.plots_lat.length > 1) updateInnerHTML('latestCardsTotalPlots' + '_previous_' + key, _token.plots_lat.length + " Plots");else updateInnerHTML('latestCardsTotalPlots' + '_previous_' + key, _token.plots_lat.length + " Plot");
      updateInnerHTML('latestCardsTotalPlotsSize' + '_previous_' + key, _token.plots_lat.length * PLOT_SIZE + "m<sup>"); //updateInnerHTML('latestCardsEmpireScore' + '_previous_' + key, _token.empire_score);
...
function updateSearchDisplay() {
  ...
  var _size = Object.keys(tokens).length;
  var _currentPageDisplayed = 0;
  var _searchOwner = document.getElementById('searchCardOwner').value; //if(!window.tronWeb.isAddress(_searchOwner))
  //  _searchOwner = "";

  var _searchName = document.getElementById('searchCardName').value;
  var _searchMinTRX = document.getElementById('searchMinTrx').value;
  if (isNaN(parseFloat(_searchMinTRX))) _searchMinTRX = "";
  var _searchMaxTRX = document.getElementById('searchMaxTrx').value;
  if (isNaN(parseFloat(_searchMaxTRX))) _searchMaxTRX = "";

  for (var c = _size - 1; c > -1; c--) {
    var _key = Object.keys(tokens)[c];
    var _token = tokens[_key]; //console.log("_token", tokens[key]);
    ...
    */
    ...
    if (_searchOwner != "") {
      if (window.tronWeb.address.fromHex(tokens[_key].token_owner).toUpperCase() == _searchOwner.toUpperCase()) {
        validToken = true;
      } else {
        validToken = false; //continue;
      }
    }

    if (_searchName != "") {
      if (tokens[_key].name.toUpperCase().indexOf(_searchName.toUpperCase()) > -1) {
        validToken = true;
      } else {
        validToken = false;
      }
    ...
    if (_searchMinTRX != "") {
      if (parseFloat(window.tronWeb.fromSun(tokens[_key].current_value.toString())) >= parseFloat(_searchMinTRX)) {
        validToken = true;
      } else {
        validToken = false;
      }
    ...
    if (_searchMaxTRX != "") {
      if (parseFloat(window.tronWeb.fromSun(tokens[_key].current_value.toString())) <= parseFloat(_searchMaxTRX)) {
        validToken = true;
      } else {
        validToken = false;
      }
    ...
    if (validToken) {
      if (_currentPageDisplayed > 17) {
        ...
        $("#searchContainer").append("<div class='grid-x grid-margin-x hide' id='searchResults" + _currentPage + "'' style='width: 100%; position: relative; top: 110px; ''></div>"); //$("#searchPagination").append("<li><a href='#' aria-label='Page " + _currentPage + "'>" + _currentPage + "</a></li>");
      } //Object.keys(tokens).forEach(function(key,index){
      //let _token = tokens[key];


      var _newCard = userCardsFull.replace(/\[\]/g, _key);

      $("#searchResults" + _currentPage).append('<div class="cell medium-2 small-6">' + _newCard + "</div>");
      $('#userCardHeader' + _key).css("background-color", '#' + _token.token_owner.substring(_token.token_owner.length - 6));
      $('#lastestCardImg' + _key).css("background-image", "url(https://api.planetcrypto.app/tronImg/" + _key + ")");
      $('#lastestCardImgBack2' + _key).css("background-image", "url(https://api.planetcrypto.app/tronImg/" + _key + ")");
      updateInnerHTML('userCardsName' + _key, _token.name);
      updateInnerHTML('userCardsCardOwner' + _key, window.tronWeb.address.fromHex(_token.token_owner).substring(0, 15) + "...");
      updateInnerHTML('userCardsStartPoint' + _key, hardFixed(_token.plots_lat[0] / 1000000, 4) + "," + hardFixed(_token.plots_lng[0] / 1000000, 4));
      if (_token.plots_lat.length > 1) updateInnerHTML('userCardsTotalPlots' + _key, _token.plots_lat.length + " Plots");else updateInnerHTML('userCardsTotalPlots' + _key, _token.plots_lat.length + " Plot");
      updateInnerHTML('userCardsTotalPlotsSize' + _key, _token.plots_lat.length * PLOT_SIZE + "m<sup>"); //updateInnerHTML('userCardsEmpireScore' + key, _token.empire_score);

      updateInnerHTML('userCardsBadge' + _key, pad(_token.empire_score, 5));
      updateInnerHTML('userCardsCost' + _key, window.tronWeb.fromSun(_token.current_value.toString()).substring(0, 6));
      document.getElementById('userCardFull' + _key).value = _key;
...
function updateUserCards() {
  ...
  if (latestStart < 0) latestStart = 0;
  playerStats = [];
  $("#latestCards").empty();
  $("#latestCards2").empty();
  $("#userLostCards").empty();
  $("#userLostCards2").empty(); //console.log("updateUserCards", tokens);
  ...
  Object.keys(tokens).forEach(function (key, index) {
    ...
    if (!isReadonly) {
      if (tokens[key].token_owner.toUpperCase() == userWalletAddress.toUpperCase()) {
        ...
        var existingDisplay;
        if (document.getElementById('userCardFull' + key)) existingDisplay = document.getElementById('userCardFull' + key).value;

        if (existingDisplay) {} else {
          var _newCard = userCardsFull.replace(/\[\]/g, key);

          if (!isMobile) $("#userCards").prepend(_newCard);else $("#userCards2").prepend(_newCard);
        }
        ...
        if (_token.plots_lat.length > 1) updateInnerHTML('userCardsTotalPlots' + key, _token.plots_lat.length + " Plots");else updateInnerHTML('userCardsTotalPlots' + key, _token.plots_lat.length + " Plot");
        updateInnerHTML('userCardsTotalPlotsSize' + key, _token.plots_lat.length * PLOT_SIZE + "m<sup>"); //updateInnerHTML('userCardsEmpireScore' + key, _token.empire_score);
    ...
    if (index == latestStart) {
      ...
      if (existingDisplay > 0) {} else {
        var _newCard = latestCardsFull.replace(/\[\]/g, key);

        if (!isMobile) $("#latestCards").prepend(_newCard);else {
      ...
      if (_token.plots_lat.length > 1) updateInnerHTML('latestCardsTotalPlots' + key, _token.plots_lat.length + " Plots");else updateInnerHTML('latestCardsTotalPlots' + key, _token.plots_lat.length + " Plot");
      updateInnerHTML('latestCardsTotalPlotsSize' + key, _token.plots_lat.length * PLOT_SIZE + "m<sup>"); //updateInnerHTML('latestCardsEmpireScore' + key, _token.empire_score);
      ...
      document.getElementById('latestCardFull' + key).value = key; //console.log("Adding latest", key, _newCard);
  ...
  for (var sortedIdx = 0; sortedIdx < _sortedPlayers.length; sortedIdx++) {
    ...
    if (sortedIdx == 0) _row += '<span class="large badge info" style="width: 40px; height: 40px; font-size: 1.3em;"><i class="fi-trophy" style="color: gold; font-weight: bold;"></i></span>';else _row += '<center><span class="small badge warning">' + (sortedIdx + 1) + '</span></center>'; //_row += '</td><td width="450"><a href="https://tronscan.org/#/address/' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" target="_new">' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '</a></td>';

    _row += '</td><td width="450"><a href="./tronCards.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '</a></td>'; //var _mobileRow = '<tr><td width="50"><span class="small badge alert">' + (sortedIdx+1) + '</span></td><td width="450"><a href="https://tronscan.org/#/address/' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" target="_new">' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress).substring(0,7) + '...</a></td>';
...
userCardsFull += '                                    <img class="icon-size"/> <strong><span id="userCardsTotalPlots[]">0</span>, <strong><span id="userCardsTotalPlotsSize[]">0m</span><sup>2</sup></strong>&nbsp;'; //userCardsFull+= '                                    <img class="icon-ruler"/> <strong><span id="userCardsTotalPlotsSize[]">0m</span><sup>2</sup></strong><br/>';
...
latestCardsFull += '                                    <img class="icon-size"/> <strong><span id="latestCardsTotalPlots[]">0</span>, <span id="latestCardsTotalPlotsSize[]">0m</span></strong>&nbsp;'; //latestCardsFull+= '                                    <img class="icon-trophy"/> <strong>+<span id="latestCardsEmpireScore[]">0</span></strong><br/>';
...
latestCardsFull += '                      </div>';
latestCardsFull += '                    </div>'; //<img src="" id="smallCardImg[]" class="smallCardImg" align="right"/>
...
function googleTranslateElementInit() {
  ...
  $('#google_translate_element').on("click", function () {
    ...
    $("iframe").contents().find('.goog-te-menu2-item-selected').css('display', 'none'); // Change menu's padding

    $("iframe").contents().find('.goog-te-menu2').css('padding', '0px'); // Change the padding of the languages

    $("iframe").contents().find('.goog-te-menu2-item div').css('padding', '20px'); // Change the width of the languages

    $("iframe").contents().find('.goog-te-menu2-item').css('width', '100%');
    $("iframe").contents().find('td').css('width', '100%'); // Change hover effects
    ...
    $("iframe").contents().find('.goog-te-menu2').css('border', 'none'); // Change the iframe's box shadow

    $(".goog-te-menu-frame").css('box-shadow', '0 16px 24px 2px rgba(0, 0, 0, 0.14), 0 6px 30px 5px rgba(0, 0, 0, 0.12), 0 8px 10px -5px rgba(0, 0, 0, 0.3)'); // Change the iframe's size and position?
...
function processJumpToLocationEvent(_this) {
  ...
  if (valueSelected.substring(0, 5) == 'token_id') {
    ...
    mymap.flyTo(L.latLng(valueSelected.split(",")[0], valueSelected.split(",")[1]), valueSelected.split(",")[2]);
  }
}
/*
  share_url = 'https://planetcrypto.app/trxToken/' + _token_id;
...
function strToBytes32Array(_in) {
  ...
  for (var c = 0; c < _in.length; c = c + 2) {
    _array.push(hexToDec((_in[c] + _in[c + 1]).toString()));
  }

  if (_array.length < 32) {
    var _max = 32 - _array.length;

    for (var _c = 0; _c < _max; _c++) {
      _array.push("00");
    }
  }
  /*
...
function destroyConfirm() {
  if (existingCardOwned) $('#destroyConfirm').foundation('open');else $('#destroyConfirmUnowned').foundation('open');
}
...
function moveEndEvent(_zoomLvl) {
  ...
  if (_zoomLvl > 16) {
    ...
    if (hardFixed(lightGrids[_zoomLvl].options.cellSize, 2) != hardFixed(getPixelSize(PLOT_SIZE), 2)) {
      // needs a reset
    ...
    if (err.error.indexOf("Not enough energy") > -1) {
      ...
      if (err.error.indexOf("bandwidth") > -1) {
        ...
        if (err.error.indexOf('Cannot find result in solidity node') > -1) {
          ...
          if (genericMsg) genericMsg();else showGenericTronErr();
        }
      }
    }
  } catch (e) {
...
function showNotEnoughEnergy() {
  ...
  showError(_msg, "Not Enough Energy"); //resetSelectedRects();
}
...
function showNotEnoughBandwidth() {
  ...
  _msg += '<p style="font-size: 10pt;">For more information please see the useful <a href="https://https://medium.com/@jacqshardy/how-to-tron-with-everdragons-c9d4de3a2497" target="_new">Guide from Everdragons</a></p>';
  showError(_msg, "Not Enough Bandwidth"); //resetSelectedRects();
}
...
function showGenericTronErr() {
  ...
  _msg += '<p style="font-size: 10pt;"><b>If you continue to receive this message and you are sure your account is correct and valid please refresh this page and try again</b></p>';
  showError(_msg, "Unable to Process Transaction"); //resetSelectedRects();
}
...
function solidityEncodeAddr(_in) {
  if (_in !== '') {
    if (window.tronWeb.isAddress(_in)) {
      return "0x" + window.tronWeb.address.toHex(_in).substring(2); //0xa10645c5e4edb159d8a449d765f639ff69cad3a2
      // should be: 
