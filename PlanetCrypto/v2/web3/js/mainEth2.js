...
var PlanetCryptoControllerContract;
...
var contractAddress = "0xb3379466da5cdac8cf9f2ad45302e2bcfb4dab4b"; // ropsten


var controllerContractAddress = "0x4d410a06c78e3d8e07adc35fe887a5b696f3c812";

var coinsContractAddress = "0x7f65245597bf659d92a3755eda5a8269903c3a3a"; // ropsten
// utils = 0x3a076f5597c74a0d0fea0b058c1bafeac037e64d

var empireBonusAddress = "0xa9e4f9b598e386c0ce5a78fd5eb358d6a94777d9"; // ropsten
...
var share_title = 'Planet Crypto ~ Etheruem Crypto Collectible Game ~ Own the World!';
...
window.addEventListener('load', function(){
	...
	//isMainGame
	if(window.location.href.indexOf("ethCards2.htm") > -1) {
		isMainGame = false;
	...
	async function setupEth(){
	    ...
	    if (window.ethereum) {
			...
			try {
				...
				gameConnection = new Web3(window.ethereum);
				_startFromHere = true;
			} catch (error) {
	...
	if(isMobile) {
		setTimeout(function(){setupEth();}, 1000);

	} else {
		setTimeout(function(){setupEth();}, 500);
...
function setupReadOnlyWeb3() {
    // READ ONLY CONNECTION...
    	window.web3 = Web3;
    	web3Provider = new Web3.providers.WebsocketProvider(CONNECTION_TO_USE);
    	isReadonly = true;
    	console.log("provider:", web3Provider);
    
    	web3Provider.on('connect',function(e){
      	gameConnection = new Web3(web3Provider);
      	//initContract();
    });

    web3Provider.on('error', function(e){
      console.error('WS Error', e);
      reconnectProvider(0);
    });

    web3Provider.on('end', function(e) {
      reconnectProvider(0);
    });
}
...
function getUserWalletAddress() {
  gameConnection.eth.getCoinbase((err, res) => {
    var output = "";
...
function startApp() {
  //if(isMainGame) {
	...
	async function setupContracts() {
		...
		try{
    		PlanetCryptoControllerContract = await new gameConnection.eth.Contract(planetCryptoControllerABI, controllerContractAddress);
			console.log("PlanetCryptoControllerContract Setup");


    	}	
    	catch(err) {
    		contractSetupHasError = true;
    		console.log("PlanetCryptoControllerContract Setup Err:", err);

			var _msg = ""
			_msg += '<p style="font-size: 10pt;"><i>Unable to connect to the network</i></p>';
			_msg += '<p style="font-size: 10pt;"><b>You were unable to connect to the Eth Network</b></p>';
			_msg += '<p style="font-size: 10pt;">Please ensure you are logged into your Eth Wallet and then refresh the page to try again.</p>';
			showError(_msg, "Error connecting to Tron Network");
    	}

    	try{
    		PlanetCryptoContract = await new gameConnection.eth.Contract(planetCryptoABI, contractAddress);
    	...
    	try{
    		PlanetCryptoCoinContract = await new gameConnection.eth.Contract(planetCryptoCoinABI, coinsContractAddress)
    	}catch(err){
    		contractSetupHasError = true;
    		console.log("PlanetCryptoCoin Setup Err:", err);
...
//
    	...
    	try{
    		PlanetCryptoEmpireBonusContract = await new gameConnection.eth.Contract(planetCryptoEmpireBonusABI, empireBonusAddress)
    	}catch(err){
    		contractSetupHasError = true;
    		console.log("PlanetCryptoBonus Setup Err:", err);
...
function showExistingCard(_token_id, _flyTo){
	...
	*/
	...
	if(isBETA)
		$('#existingCardImg').css("background-image", "url(https://api.planetcrypto.app/tronImgShasta/" + _token_id + ")"); 
	else
...
function processPreviousCardsToken(_token, key){
	...
	if(_token.token_owner.toUpperCase() != userWalletAddress.toUpperCase()){
		...
		if(usedToOwn){
			...
			if(isBETA)
				$('#lastestCardImg'  + '_previous_' + key).css("background-image", "url(https://api.planetcrypto.app/tronImgShasta/" + key + ")"); 
			else
...
*/
		...
		if(validToken){
			...
			if(isBETA)
				$('#lastestCardImg' + key).css("background-image", "url(https://api.planetcrypto.app/tronImgShasta/" + key + ")"); 
			else
