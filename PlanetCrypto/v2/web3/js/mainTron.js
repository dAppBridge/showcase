...
var contractAddress = "TGjFMawd8rhyeSNaihwPTZwgN5u2sWiZXY"; // shasta
...
async function getTokenDetails(_token_id, _shortDetails, _processor) {
	let hasError = false;
	let results = await PlanetCryptoContract.getTokenEnhanced(_token_id, _shortDetails).call().catch(function(err){
...
function getEarnings(_callback) {
	...
	PlanetCryptoContract.total_invested().call().then(function(results){
		...
		updateInnerHTML('totalInvestedMobile', window.tronWeb.fromSun(total_invested.toString()).substring(0,8));
		updateInnerHTML('totalInvested', window.tronWeb.fromSun(total_invested.toString()).substring(0,8));
	});
	PlanetCryptoContract.total_earned().call().then(function(results){
		total_earned = new BigNumber(results.toString());
		updateInnerHTML('totalEarningsMobile', window.tronWeb.fromSun(total_earned.toString()).substring(0,8));
		updateInnerHTML('totalEarnings', window.tronWeb.fromSun(total_earned.toString()).substring(0,8));
	});
	if(!isReadonly){
...
//dailyAwardedIn
  ...
  let _endTime = currentDayBonus.startTS.times(1000).toNumber() + (60 * 60 * 24 * 1000); 
...
console.log("CLEARING2");
	...
	for(var sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		...
		_row += '</td><td width="200"><a href="./tronCards.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '</a></td>';
		...
		var _mobileRow = '<tr><td width="100"><span class="small badge alert">' + (sortedIdx+1) + '</span></td><td width="450"><a href="./tronCards.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress).substring(0,7) + '...</a></td>';
		...
		_mobileRow += '<td width="100" style="text-align:center;">' + _displayLandSize + _displayLandSizeUnit + ' sq.</td>';
...
function updateLeaderboard() {
	...
	for(var sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		...
		_mobileRow += '<td width="100">' + _displayLandSize + _displayLandSizeUnit + ' sq.</td>';
		...
		_mobileRow += '<td width="100">' + _sortedPlayers[sortedIdx].totalEmpireScore + '</td></tr>'
...
var userCardsFull = '<div class="userCardOuter" id="userCardFull[]" onclick="showExistingUserCard(this);"><div class="userCardsItemFull" >';
