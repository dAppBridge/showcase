...
var PlanetCryptoEmpireBonusContract;
...
//var contractAddress = "TCqF2AVtNrciZLBqhoithaqEuotDqT4Mit"; // shasta NEW

//var coinsContractAddress = "TPcmPD1pTSnk8NVDbCpW26im69PLMFfhrk"; // shasta live

var coinsContractAddress = "TYE6g4YAT2dBYgPEkUQ8vRAHJePU3pBWXz"; // live active

//var empireBonusAddress = "TUkUPmKj1kGBtxx4bBWKyX4c1D5uoxGpuV"; // shasta
var empireBonusAddress = "TGNJp4HDScV3t2qjArF1x9U5kFcUCFLNrH"; // mainnet
...
var isFounderPlayer = false;
var founder = {
	totalEmpireScore: new BigNumber(0),
	totalLand: new BigNumber(0),
	sunOutstanding: new BigNumber(0),
	sunEarned: new BigNumber(0),
	activated: false
};
...
/*
	var _newFirstPlay = Cookies.get('firstPlay')||"";
	if(_newFirstPlay.length > 0) {
		isFirstPlay = false;
	} else {
		// is firstPlay
		isFirstPlay = true;
		Cookies.set('firstPlay', false, { expires: 30, path: '/' });
	}
*/

	//isMainGame
	if(window.location.href.indexOf("tronCards2.htm") > -1) {
		isMainGame = false;
...
try{
		$(".off-canvas").on("opened.zf.offcanvas", function(e) {
			...
			$('#basemaps-wrapper').addClass('hide');
			$('#jumpToLocationWrapper').addClass('hide');
...
try{
		$(".off-canvas").on("closed.zf.offcanvas", function(e) {
			...
			$('#basemaps-wrapper').removeClass('hide');
			$('#jumpToLocationWrapper').removeClass('hide');
		});
}
catch(e){}
	}
	...
	if(isCard == "1") {
		...
		$('#buyBonusCard0').click(function(){

			buyBonus(0);
		});
		$('#buyBonusCard1').click(function(){
			buyBonus(1);
		});
		$('#buyBonusCard2').click(function(){
			buyBonus(2);
		});
		$('#buyBonusCard3').click(function(){
			buyBonus(3);
		});
		$('#buyBonusCard4').click(function(){
			buyBonus(4);
		});
		...
		$('#copyRefMobile').click(function(){
			$.toast({
				heading: 'Ref Link Copied',
				text: 'Your referral URL is copied to your clipboard!',
				icon: 'info',
				stack: 10,
		...
		$('#unlockVanity').click(function(){updateVanity();});
		$('#unlockVanitymobile').click(function(){updateVanitymobile();});
		$('#playerFlag').click(function(){updatePlayerFlag();});
		$('#playerFlagMobile').click(function(){updatePlayerFlag();});
		$('#updateFlagBtn').click(function(){savePlayerFlag()});
...
//			$('#firstPlay').removeClass('hide');
//			$('#firstPlay').foundation('open');

			if(isFounderPlayer){
				//$("#founderPlayerDiv").foundation('open');
					updateInnerHTML('founderLandSize', founder.totalLand.toNumber() * 20);
					updateInnerHTML('founderEmpireScore', founder.totalEmpireScore.toNumber());
				//	updateInnerHTML('founderSUN', window.tronWeb.fromSun(founder.sunOutstanding.toString()).substring(0,8) + " TRX");

					$("#founderPlayerDiv").foundation('open');

				/*				
				if(!founder.activated) {
					updateInnerHTML('founderLandSize', founder.totalLand.toNumber() * 20);
					updateInnerHTML('founderEmpireScore', founder.totalEmpireScore.toNumber());
					updateInnerHTML('founderSUN', window.tronWeb.fromSun(founder.sunOutstanding.toString()).substring(0,8) + " TRX");
					...
					// get sun earned so far...
					PlanetCryptoContract.founderTaxEarned().call().then(function(results){
						console.log("FOUNDER EARNINGS:", results);
						let _newEarned = results._newEarned.toNumber();
						let _existingEarned = results._existingEarned.toNumber();

						updateInnerHTML('founderLandSize2', founder.totalLand.toNumber() * 20);
						updateInnerHTML('founderEmpireScore2', founder.totalEmpireScore.toNumber());
						updateInnerHTML('founderSUN2', window.tronWeb.fromSun(founder.sunOutstanding.minus(results._newEarned).toString()).substring(0,8) + " TRX");
						updateInnerHTML('founderSUNEarned', window.tronWeb.fromSun(_newEarned + _existingEarned).substring(0,8) + " TRX");

						founder.sunOutstanding = founder.sunOutstanding.minus(results._newEarned);

						$("#founderPlayerDiv2").foundation('open');
		...
		$('#buyEmpire').click(function() { buyEmpire(); });
		$('#buyEmpireMobile').click(function() { buyEmpire(); });
		...
		$('#existingCardOwner').click(function(){
			...
			document.location.href = './tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(document.getElementById('existingCardOwner').value);
		...
		$('#newCardStartOwner').click(function(){
			...
			document.location.href = './tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(document.getElementById('newCardOwnerAddr').value);
		...
		$('#mapViewBTN').click(function(){
			document.location.href = './flatearth.htm';
		});
		$('#searchBTN').click(function(){
			doSearch();
		});
		$('#mobileSearchBtn').click(function(){
			document.location.href = './tronCards2.htm';
		});
		$('#tab8').click(function(){
			document.location.href = './tronCards2.htm';
		});
		$('#founderActivate').click(function(){
			activateFounderPlayer();
		});
...
async function getTokenDetails(_token_id, _shortDetails, _processor) {
	let hasError = false;
	let results = await PlanetCryptoContract.getTokenEnhanced2(_token_id).call().catch(function(err){
...
var total_invested = new BigNumber(0);
var total_earned = new BigNumber(0);
var total_available = new BigNumber(0);
var hasEarnings = false;

function getEarnings(_callback) {
	
	PlanetCryptoContract.total_invested().call().then(function(results){
		total_invested = new BigNumber(results.toString());
		updateInnerHTML('totalInvestedMobile', window.tronWeb.fromSun(total_invested.plus(106574437500).toString()).substring(0,8));
		updateInnerHTML('totalInvested', window.tronWeb.fromSun(total_invested.plus(106574437500).toString()).substring(0,8));
	});
	PlanetCryptoContract.total_earned().call().then(function(results){
		total_earned = new BigNumber(results.toString());


		updateInnerHTML('totalEarningsMobile', window.tronWeb.fromSun(total_earned.plus(63156420203).toString()).substring(0,8));
		updateInnerHTML('totalEarnings', window.tronWeb.fromSun(total_earned.plus(63156420203).toString()).substring(0,8));
	});
	if(!isReadonly){
		PlanetCryptoContract.taxEarningsAvailable().call().then(function(results){
			if(results) {


				total_available = new BigNumber(results.toString());
				if(total_available.gt(0))
					hasEarnings = true;

				updateInnerHTML('yourTaxAvailableMobile', window.tronWeb.fromSun(total_available.toString()).substring(0,8));
				updateInnerHTML('yourTaxAvailable', window.tronWeb.fromSun(total_available.toString()).substring(0,8));
			} else {
				updateInnerHTML('yourTaxAvailableMobile', window.tronWeb.fromSun(total_available.toString()).substring(0,8));
				updateInnerHTML('yourTaxAvailable', window.tronWeb.fromSun(total_available.toString()).substring(0,8));
			}
...
*/
	updateDayStats(_callback);
}

var earningsUpdateQueued = false;

function processEarningsUpdate() {
	if(!earningsUpdateQueued){
		earningsUpdateQueued = true;
		setTimeout(function() {
			
			getEarnings();
			earningsUpdateQueued = false;
		},2000);
...
function activateFounderPlayer() {
	$("#founderPlayerDiv").foundation('close');
	//$("#founderPlayerDiv2").foundation('close');
return;
/*
	showProcessing();
	$.toast({
	...
	PlanetCryptoContract.activateFounderPlayer().send().catch(function(err){
		hasError = true;
		hideProcessing();
		...
		if(err == "Confirmation declined by user"){
			...
			processGenericErrors(err, showGenericTronErr);
		...
		try {
			...
			if(!hasError){
		      	...
		      	var _msg = ""
		      	_msg += '<p style="font-size: 10pt;"><strong>Account Active!</p>';


		      	_msg += '<p style="font-size: 10pt;">You\'ll now start to receive Dividends up to 150% of the value of your original land!</p><br/><br/>';
		      	...
		      	showError(_msg, "Activation Complete");
...
// 6 = card name change, 7 = card image change
...
// 6 = card name change, 7 = card image change
	...
	if(logResult.result.event_type){



		//if(logResu =='landPurchased'){
		if(logResult.result.event_type == "2") {
			...
			if(isInit == true) {
				...
				var _msg = 'By: ' + window.tronWeb.address.fromHex(logResult.result.player).substring(0,15) + "..." + "<br/>";
				...
				if(logResult.result.player.toUpperCase() == userWalletAddress.toUpperCase()){
		...
		if(showToast) {
			...
			if(logResult.result.event_type == "3") {
				...
				if(parseInt(logResult.result.amnt) > 0) {
					...
					_msg += "To: " + logResult.result.size + " Players";
			...
			if(logResult.result.event_type == "0") {
				var _msg = window.tronWeb.address.fromHex(logResult.result.player).substring(0,15) + "...<br/>";
			...
			if(logResult.result.event_type == "1"){	
				var _msg = window.tronWeb.address.fromHex(logResult.result.player).substring(0,15) + "...<br/>";
				_msg += "has just mined PlanetCOINs:<br/>"
				_msg += logResult.result.amnt.toString() + " PlanetCOINS<br/>";
				_msg += "Check the Guide for info on how you can mine PlanetCOINS!";
				$.toast({
				  heading: 'PlanetCOINs Mined!',
				  text: _msg,
				  icon: 'info',
				  stack: 10,
				  allowToastClose: true,
			...
			//if(logResult.name == 'bonusDistributed') {
			if(logResult.result.event_type == "4") {

				processEarningsUpdate();

				var _msg = window.tronWeb.address.fromHex(logResult.result.player).substring(0,15) + "...<br/>";
				_msg += "has just earned a daily bonus of:<br/>"
				_msg += window.tronWeb.fromSun(logResult.result.amnt.toString()) + " TRX";
				$.toast({
				  heading: 'Daily Bonus Earned!',
				  text: _msg,
				  icon: 'info',
				  stack: 10,
				  allowToastClose: true,
			...
			if(logResult.result.event_type == "9") {



				var _msg = window.tronWeb.address.fromHex(logResult.result.player).substring(0,15) + "...<br/>";
				if(logResult.result.token_id == 0){
					_msg += "has just bought a Amethyst Empire Boost!<br/>"	
				}
				if(logResult.result.token_id == 1){
					_msg += "has just bought a Ruby Empire Boost!<br/>"	
				}
				if(logResult.result.token_id == 2){
					_msg += "has just bought a Emerald Empire Boost!<br/>"	
				}
				if(logResult.result.token_id == 3){
					_msg += "has just bought a Gold Empire Boost!<br/>"	
				}
				if(logResult.result.token_id == 4){
					_msg += "has just bought a Diamond Empire Boost!<br/>"	
				}

				_msg += "Boosting their score by: " + logResult.result.empire_score;

				$.toast({
				  heading: 'Empire Boost!',
				  text: _msg,
				  icon: 'info',
				  bgColor : 'orange',
				  stack: 10,
				...
				for(let i=0; i< players.length; i++) {
					if(players[i].playerAddress == logResult.result.player) {
						//console.log("BEFORE PLAYER:", players[i]);
						players[i].totalEmpireScore = parseInt(players[i].totalEmpireScore) + parseInt(logResult.result.empire_score);
						//console.log("AFTER PLAYER:", players[i]);
						//console.log("RESULT:", logResult);

						players[i].additionalEmpireScore = parseInt(players[i].additionalEmpireScore) + parseInt(logResult.result.empire_score);

						if(players[i].playerAddress.toUpperCase() == userWalletAddress.toUpperCase()){
							currentUserEmpireScore = players[i].totalEmpireScore;
							updateInnerHTML('yourEmpireScore', players[i].totalEmpireScore);
							updateInnerHTML('yourEmpireScoreMobile', players[i].totalEmpireScore);
						}
						break;
					}
				}

				for(let i=0; i< currentDayEmpireScores.length; i++) {
		
					if(currentDayEmpireScores[i].player.substring(2).toUpperCase() == logResult.result.player.substring(2).toUpperCase()) {

						currentDayEmpireScores[i].empireScore = parseInt(currentDayEmpireScores[i].empireScore) + parseInt(logResult.result.empire_score);
				...
				//updateDayStats(updateLeaderboard);

				updateLeaderboard();


    			let _boostKey = logResult.result.token_id.toString() + ":" +logResult.result.timestamp.toString() + ":" + logResult.result.player.toString();
				...
				let _newBoostCard = _card.replace(/\[\]/g, _boostKey)
				if(!isMobile)
					$("#latestCards").prepend(_newBoostCard);
				else{
					$("#latestCards2").prepend(_newBoostCard);
				}
				document.getElementById('boostCardFull' + _boostKey).value = _boostKey;

				updateInnerHTML('boostScore' + _boostKey, '+' + logResult.result.empire_score.toString() + ' EMPIRE SCORE');
				updateInnerHTML('bonusCost' + _boostKey, window.tronWeb.fromSun(logResult.result.amnt.toString()).substring(0,8) + " TRX");
		...
		if(logResult.result.event_type == "5"){
				...
				tokens[logResult.result.token_id].buy_history.push(
						{
							...
							from: logResult.result.player.replace("0x", "41")//,
			...
			if(isInit) {
				...
				//try{
				_encodedName = window.tronWeb.fromHex(logResult.result.data);
				//} catch(e) {
					//_encodedName = window.tronWeb.toHex(logResult.result.data);
				//}

			
				var _msg = "Name: " + _encodedName + "<br/>";
				_msg += 'By: ' + window.tronWeb.address.fromHex(logResult.result.to).substring(0,10) + "..." + "<br/>";
				_msg += 'From: ' + window.tronWeb.address.fromHex(logResult.result.player).substring(0,10) + "..." + "<br/>";
				_msg += 'For: ' + window.tronWeb.fromSun(logResult.result.bought_at.toString()).substring(0,8) + " TRX<br/>";
				_msg += 'Empire Score: +' + logResult.result.empire_score;
			...
			if(tokens[logResult.result.tokenId]) {
				...
				*/
				...
				if(isInit) {
					...
					// check if it is this players new card...
					if(logResult.result.to.toUpperCase() == userWalletAddress.toUpperCase()){
						...
						updateInnerHTML('latestCardsCardOwner' + logResult.result.tokenId, logResult.result.to.substring(0,15) + "...");
					...
					if(logResult.result.from.toUpperCase() == userWalletAddress.toUpperCase()){
						...
						_tokens[logResult.result.tokenId]._wasPreviouslyOwned = true;
						...
						if(logResult.result.to == NULL_ADDR) {
							...
							// add to previous cards (With updated details)
							if(_tokens[logResult.result.tokenId]._wasPreviouslyOwned)
								processPreviousCardsToken(tokens[logResult.result.tokenId], logResult.result.tokenId);
...
var udpateClocksRunning = false;

var updateChecksRunning = false;
function updateChecks() {
	updateChecksRunning = true;

	//
	let _now = new Date().getTime();
	let _diff = (_now - _lastTokenCheck); 
	//console.log("LAST TOKEN CHECK:", _diff);
...
//dailyAwardedIn
  ...
  if(isBETA)
  	_endTime = currentDayBonus.startTS.times(1000).toNumber() + (60 * 60 * 24 * 1000); 
  else
  	_endTime = currentDayBonus.startTS.times(1000).toNumber() + (60 * 60 * 24 * 1000); 

  let _diff = (_endTime - _now); 
  if(_diff < 0) {
    updateInnerHTML('dailyAwardedIn', "Awarding.."); 
    updateInnerHTML('dailyAwardedIn2', "Awarding.."); 
    updateInnerHTML('dayBonusTime', "Awarding..");
    updateInnerHTML('mobileLatestCards4', "");
    updateDayStats(updateLeaderboard);
    ...
    if(_diffStr2 == "00:00:00") {
    	_diffStr2 = "Awarding..";
    	updateDayStats(updateLeaderboard);
    }

    updateInnerHTML('dailyAwardedIn', _diffStr2);    
    updateInnerHTML('dailyAwardedIn2', _diffStr2);    
    updateInnerHTML('dayBonusTime', _diffStr2);
    updateInnerHTML('mobileLatestCards4', 'Bonus ' + _diffStr2);
  }
...
function vanityOwner(token, short) {
	//console.log(token);
	if(token.ownerVanity.length > 0) {
		return token.ownerVanity
	} else {
		if(short)
			return window.tronWeb.address.fromHex(token.token_owner).substring(0,10) + "...";
		else 
			return window.tronWeb.address.fromHex(token.token_owner);
	}
}
function tokenColor(token) {
	let _color = "";
	if(token.playerFlag.length > 0) {
		let _rgba = token.playerFlag.split(",");
		_color = 'rgb(' + _rgba[0] + "," + _rgba[1] + "," + _rgba[2] + ")";
	} else {
		_color = '#' + token.token_owner.substring(token.token_owner.length-6);
	}
	//console.log(token, _color);
	return _color;
}
...
function setNewCardBg() {
	...
	if(userWalletAddress.length > 5){
		...
		let _color = userWalletAddress.substring(userWalletAddress.length-6);

		PlanetCryptoContract.getPlayerFlag(userWalletAddress).call().then(function(results){

			if(results.flagSet == true){
				// 227,95,30,1
				let _rgba = uft8Tidy(window.tronWeb.toAscii(results.flagDetail)).split(",");
				//console.log({r:  _rgba[0], g: _rgba[1], b: _rgba[2]});

				$('#playerFlag').css("background-color", 'rgb(' + _rgba[0] + "," + _rgba[1] + "," + _rgba[2] + ")");
				$('#playerFlagMobile').css("background-color", 'rgb(' + _rgba[0] + "," + _rgba[1] + "," + _rgba[2] + ")");

				if(isMainGame)
				$('#mycolorpicker').setColor({r:  _rgba[0], g: _rgba[1], b: _rgba[2]});

				$('#newCardHeader').css("background-color", 'rgb(' + _rgba[0] + "," + _rgba[1] + "," + _rgba[2] + ")");

			} else 
			{

				$('#playerFlag').css("background-color", '#' + _color);
				$('#playerFlagMobile').css("background-color", '#' + _color);

				if(isMainGame)
				$('#mycolorpicker').setColor(_color);

				$('#newCardHeader').css("background-color", '#' + _color);

			}
			//console.log("FLAG:", uft8Tidy(window.tronWeb.toAscii(results.flagDetail)));
		...
		document.getElementById('newCardOwnerAddr').value = window.tronWeb.address.fromHex(userWalletAddress);
		updateInnerHTML('newCardOwnerAddr', window.tronWeb.address.fromHex(userWalletAddress).substring(0,10) + "...");
...
//
    	...
    	try{
    		PlanetCryptoEmpireBonusContract = await window.tronWeb.contract(planetCryptoEmpireBonusABI_tron.abi, empireBonusAddress)
    	}catch(err){
    		contractSetupHasError = true;
    		console.log("PlanetCryptoBonusContract Setup Err:", err);
    	...
    	if(!isReadonly){
    		...
    		if(isMainGame){
    			// check if player is founder player...
/*
    			PlanetCryptoContract.getFounderPlayer(userWalletAddress).call().then(function(results){
    				console.log("FOUNDER:", results);


					if(parseInt(results.totalEmpireScore.toString()) > 0) {
						console.log("FOUNDER OK");
						isFounderPlayer = true;
						founder.totalEmpireScore = new BigNumber(results.totalEmpireScore);
						founder.totalLand = new BigNumber(results.totalLand);
						founder.sunOutstanding = new BigNumber(results.sunOutstanding);
						founder.sunEarned = new BigNumber(results.sunEarned);
						founder.activated = results.activated;
...
function checkForNewTokens(isInit){
	...
	_lastTokenCheck = new Date().getTime();
	...
	PlanetCryptoContract.tokenIDCount().call().then(function(result){
		...
		if(_tokenIDCount > lastTokenIDCount) {
			...
			console.log("TOTAL:", _tokenIDCount, lastTokenIDCount);
			...
			let _currentCount = 0;
			for(let c=lastTokenIDCount; c<_tokenIDCount; c++) {
				let _token_id = c+1;
				...
				if(tokens[_token_id]) {
					if(_token_id == _tokenIDCount) {
						console.log("Processed last token at setup2");
					...
					getTokenDetails(_token_id, false, function(results){
						//console.log("token:", _token_id, results);
						if(isInit)
							_initTotalReceived++;
						...
						//		updateInnerHTML('startCards', totalCards);
							...
							try{
								tokens[_token_id] = {
									...
									playerFlag: uft8Tidy(window.tronWeb.toUtf8(results.playerFlag)),
									ownerVanity: results.ownerVanity
								};
								let _wasPreviouslyOwned = false;
								for(let i=0; i< results.previous_owners.length;i++){
									tokens[_token_id].buy_history.push({token_owner: results.previous_owners[i].replace("0x", "41")});
									if(results.previous_owners[i].replace("0x", "41").toUpperCase() == userWalletAddress.toUpperCase()){
										_wasPreviouslyOwned = true;
									}
								}
								tokens[_token_id]._wasPreviouslyOwned = _wasPreviouslyOwned;
							//	console.log("TOKEN:", tokens[_token_id]);
							...
							if(isMainGame) {
								...
								if(fireGameDataReadyCounter == 0 || c == _tokenIDCount-1){
									if(c== _tokenIDCount-1) {
										// at end so add in the final marker...
										...
										//_marker.addTo(mymap);
										tokens[_token_id]._markerAdded = true;
										if(!isInit){
									...
									//console.log("PING", _markersToAdd);
						...
						if(_initTotalReceived == _tokenIDCount) {
							console.log("Processed last token at setup3");


							$("#loadingDiv").addClass('hide');
							if(_markersToAdd.length > 0)
								markerCluster.addLayers(_markersToAdd);
							//startFireGameDataReady();
							//console.log(_markersToAdd);
...
//console.log("results", results);
	...
	if(_token_id == _tokenIDCount) {
		console.log("Processed last token at setup1");
		$("#loadingDiv").addClass('hide');
		startFireGameDataReady();
...
var boostCards = [];
var boostCardsShown = 0;

function checkForBoostCards() {
	boostCards = [];
	boostCardsShown = 0;

	PlanetCryptoEmpireBonusContract.playerBonusCardsLen().call().then(function(results){
		let _numBoostCards = parseInt(results.toString());
		//console.log("_numBoostCards", _numBoostCards);
		for(let i=0; i< _numBoostCards; i++) {

			PlanetCryptoEmpireBonusContract.playerBonusCards(i).call().then(function(results){

				boostCards.push({
        			pack_id: 	results.pack_id,
        			purchase_time: results.purchase_time,
        			price: 	results.price,
        			owner: results.owner,
        			empire_score: results.empire_score
				});
		...
		//	console.log("boostCard", boostCards);
		}
	});
}
...
function setupListeners() {
	...
	PlanetCryptoContract["action"]().watch(function(err,res){
		//console.log("Event Watched:", res);
		if(res)
			processLogResult(res, false, false);
	}).catch(function(err){
		console.log("Error getting action events", err);
	});
	...
	PlanetCryptoEmpireBonusContract["action"]().watch(function(err,res){
		if(res)
			processLogResult(res, false, false);
	}).catch(function(err){
		console.log("Error getting action events", err);
...
/*
	...
	PlanetCryptoContract["bonusDistributed"]().watch(function(err,res){
		console.log("Event Watched:", res);
		if(res)
			processLogResult(res, false, false);
	}).catch(function(err){
		console.log("Error getting bonusDistributed events", err);
	});
	...
	//console.log("requiresLogRead", requiresLogRead);
		...
		getTokenDetails(_token_id, false, function(results){
			...
			if(results) {
				...
				if(results.ownerVanity.length > 0){
					try{
						for(let i=0; i<players.length;i++) {
							if(players[i].playerAddress == results.token_owner) {
								players[i].vanity == results.ownerVanity;
								break;
							}
						}
					} catch(e){
...
function initStats(_callback) {
	...
	async function getCurrentPlayers() {
		...
		for(var c=1;c<currentPlayers;c++) {
			...
			PlanetCryptoContract.playersVanity(_playerObj.playerAddress).call().then(function(results){
				let _vanity = "";
				if(results.vanityStatus == true) {
					_vanity = results.vanity;
				}

				players.push({
					lastAccess: _playerObj.lastAccess,
					playerAddress: _playerObj.playerAddress,
					totalEmpireScore: _playerObj.totalEmpireScore, // includes additionalEmpireScore
					totalLand: _playerObj.totalLand,
					additionalEmpireScore: _playerObj.additionalEmpireScore,
					vanity: _vanity
				});
...
console.log("TAX:", _taxAvail);
		...
		updateInnerHTML('yourTaxAvailable', window.tronWeb.fromSun(tax_available.toString()).substring(0,8));
		...
		updateInnerHTML('yourTaxAvailableMobile', window.tronWeb.fromSun(tax_available.toString()).substring(0,8));
...
function withdrawFunds() {
	...
	//if( founder.sunOutstanding.eq(0) && 
	//	tax_available.eq(0) && tax_available2.eq(0) ) {
	if(!hasEarnings) {
...
/*
	if(tax_available3.gt(0)){
		...
		if(tax_available2.gt(0))
			...
			doWithdrawel(PlanetCryptoContract, function(){getEarnings()});		
	//}
...
function updateVanity() {
	...
	let _cost = window.tronWeb.toSun(100);
	// check if name exists...
	PlanetCryptoContract.vanityToAddress(_name).call().then(function(results){
		...
		if(results == NULL_ADDR) {
			...
			PlanetCryptoContract.buyVanity(_name).send({callValue: _cost}).catch(function(err){
				...
				try {
					...
					if(!hasError){
			          	...
			          	var _msg = ""
			          	_msg += '<p style="font-size: 10pt;"><i>Congratulations!, you now own  the name <b>' + _name + '</b>!</i></p>';
			          	...
			          	_msg += '</p>';
			          	_msg += '<p style="font-size: 10pt;">';
			          	_msg += '<code style="font-size: 8pt;">https://planetcrypto.app/?tron=' + _name + '</code>';
			          	_msg += '</p>';
			          	_msg += '<p style="font-size: 10pt;">';
			          	_msg += ' For every new player you refer on - you will earn 5% from purchases as an instant bonus!';
			          	_msg += '</p>';
...
function updateVanitymobile() {
	if(isReadonly){
		$('#notConnectedModel').foundation('open');
		return;		
	}
	let _name = document.getElementById('vanityNamemobile').value;
	let _cost = window.tronWeb.toSun(100);
...
function updatePlayerFlag() {
	if(isReadonly) {
		...
		// check if updateFlagPrice is needed...
		PlanetCryptoContract.getPlayerFlag(userWalletAddress).call().then(function(results){
			if(results.flagSet == true) {
				requiresFlagUnlock = false;
				$('#updateFlagPrice').addClass('hide');
			}
			$('#setPlayerFlagDiv').foundation('open');
...
function savePlayerFlag() {
	if(isReadonly) {
		$('#notConnectedModel').foundation('open');
		return;
	}

	let _data = $('#mycolorpicker').getColor('rgba');
	console.log("COLOR:", _data);
	
	let _encodedColor = window.tronWeb.toHex(
		_data.r.toString() + "," + 
		_data.g.toString() + "," + 
		_data.b.toString() + "," + 
		_data.a.toString());

	console.log("ENCODED COLOR:", _encodedColor);
	let _colorBytes = strToBytes32Array(_encodedColor);
	let _cost = 0;

	if(requiresFlagUnlock)
		_cost = window.tronWeb.toSun(200);
	...
	PlanetCryptoContract.updatePlayerFlag(_colorBytes).send({
		callValue: _cost
	}).catch(function(err){
		hasError = true;
		...
		try {
			...
			if(!hasError){
	          	...
	          	var _msg = ""
	          	_msg += '<p style="font-size: 10pt;"><i>Flag Color Changed!</i></p>';



	          	_msg += '<p style="font-size: 10pt;">Your properties will now be shown in your new color <strong>You will need to refresh to see them all updated!</strong></p>';

	          	_msg += '<center><button type="button" onclick="reloadPage();" class="button bordered information " data-close>RELOAD</button></center>';
	          	...
	          	_msg += '<p style="font-size: 10pt;"><b>The more players we have - the more profits you share!</b></p>';
	          	showError(_msg, "Update Complete");
...
*/
					...
					var _rect = L.rectangle(
						lightGrids[zoomLvl].cellDetails[cellKey].bounds, 
						{color: tokenColor(_secondaryLookups[c].token),
						 weight: 2, fillOpacity: 0.5});
		            ...
		            */
					...
					_msg += "<b>Owned By:</b><br/> " + vanityOwner(_secondaryLookups[c].token);
...
function createMarker(_token_id,_token, latLngPoint) {
	...
	_msg += "<b>Owner:</b><br/> " + vanityOwner(_token);
...
function showExistingCard(_token_id, _flyTo){
	...
	$('#existingCardHeader').css("background-color", tokenColor(_token));
	
	if(isMobile){
		//updateInnerHTML('existingCardOwner', window.tronWeb.address.fromHex(_token.token_owner).substring(0,10) + "...");
		updateInnerHTML('existingCardOwner', vanityOwner(_token,true));
	...
	*/
	...
	updateInnerHTML('existingCardsTotalPlotsSize', _LandSize + _LandSizeUnit);

	

	//updateInnerHTML('existingCardsEmpireScore', _token.empire_score);
	updateInnerHTML('existingCardsCost', window.tronWeb.fromSun(_token.current_value.toString()).substring(0,8) + " TRX");
	...
	updateInnerHTML('existingCardsScore', _token.empire_score);
	...
	updateInnerHTML('existingCardsName', _token.name);
	updateInnerHTML('existingCardsStartPoint', _token.plots_lat[0]/1000000 + "," + _token.plots_lng[0]/1000000);
	...
	*/
	...
	if(isBETA)
		$('#existingCardImg').css("background-image", "url(https://api.planetcrypto.app:8443/tronImgShasta/" + _token_id + ")"); 
	else
		$('#existingCardImg').css("background-image", "url(https://api.planetcrypto.app:8443/tronImg/" + _token_id + ")"); 
	...
	//share_description += ", owned by: " + window.tronWeb.address.fromHex(_token.token_owner) + " ";
	share_description += ", owned by: " + vanityOwner(_token) + " ";
	

	$("meta[property='og:description']").attr("content", share_description);

	$("meta[property='og:image']").attr("content", 'https://api.tronwin.app/tronImg/' + _token_id);
...
function setupMap() {
	...
	$('#offCanvasContent').removeClass('hide');
	...
	if(isMobile){
		// allows us to get the window size on TronWallet
		setTimeout(function(){
	...
	mymap.on('zoomend', function(ev){
		...
		if(ev.target._zoom  >= MIN_BUY_LVL) {
			if(actionMode != 'BUYSINGLE') {
				console.log("Activating buy mode...");
				startBuySingle();
			}
		} else {
...
function resetButtons() {
	...
	$('#buySingle').removeClass('buyBtnActive');
	$('#buyMultiple').removeClass('buyMultiActive');
	$('#removeSingle').removeClass('buyBtnActive');

	$('#buyEmpire').removeClass('buyEmpire');
...
function startBuyMultiple() {
	...
	actionMode = 'BUYMULTI';
  	...
  	resetButtons();
  	$('#buyMultiple').addClass('buyMultiActive');
...
function startRemoveSingle() {
	...
	actionMode = 'REMOVESINGLE';
  	...
  	resetButtons();
  	$('#removeSingle').addClass('buyBtnActive');	
...
function startBuySingle() {
	...
	actionMode = 'BUYSINGLE';
  	...
  	resetButtons();
  	$('#buySingle').addClass('buyBtnActive');
  	$('#buySingleMobile').addClass('secondary').removeClass('success');

  	ga('send', 'event', 'BuyTools', 'BuySingleOrMultiple');
}

var bonusPrice0 = new BigNumber(0);
var bonusPrice1 = new BigNumber(0);
var bonusPrice2 = new BigNumber(0);
var bonusPrice3 = new BigNumber(0);
var bonusPrice4 = new BigNumber(0);
function buyBonus(_packID){
	$('#buyEmpireDiv').foundation('close');
	...
	if(currentUserLand <1) {
		showError("You need to own at least 1 plot of land to unlock this feature.<br/><br/>Please purchase some land first.", "No Land Owned!");
		return;
	} 


	let _amnt = new BigNumber(0);
	...
	if(_amnt.lt(1)) {
		showError("Error accessing price data - please refresh and try again!", "Error");
		return;
	}
	...
	PlanetCryptoEmpireBonusContract.buyBonusPack(_packID, _ref).send({
		callValue: _amnt.toNumber()
		}).catch(function(err){
			hasError = true;
			...
			if(!hasError){
	          	...
	          	var _msg = ""
	          	_msg += '<p style="font-size: 10pt;"><i>Congratulations!, Your Bonus Pack has been Purchased!</i></p>';
...
function buyEmpire() {
	PlanetCryptoEmpireBonusContract.getBonusPackPrice(0).call().then(function(results) {
		bonusPrice0 = new BigNumber(results.toString());
		updateInnerHTML('bonusCost0', tronWeb.fromSun(results.toString()).substring(0,8));


		PlanetCryptoEmpireBonusContract.getBonusPackPrice(1).call().then(function(results) {
			bonusPrice1 = new BigNumber(results.toString());
			updateInnerHTML('bonusCost1', tronWeb.fromSun(results.toString()).substring(0,8));


			PlanetCryptoEmpireBonusContract.getBonusPackPrice(2).call().then(function(results) {
				bonusPrice2 = new BigNumber(results.toString());
				updateInnerHTML('bonusCost2', tronWeb.fromSun(results.toString()).substring(0,8));


				PlanetCryptoEmpireBonusContract.getBonusPackPrice(3).call().then(function(results) {
					bonusPrice3 = new BigNumber(results.toString());
					updateInnerHTML('bonusCost3', tronWeb.fromSun(results.toString()).substring(0,8));


					PlanetCryptoEmpireBonusContract.getBonusPackPrice(4).call().then(function(results) {
						bonusPrice4 = new BigNumber(results.toString());
						updateInnerHTML('bonusCost4', tronWeb.fromSun(results.toString()).substring(0,8));
						$('#buyEmpireDiv').foundation('open');

					}).catch(function(err) {
						console.log("err getting prices:", err);
					});
...
function updateNewCard() {
	...
	if(_rectsToBuyLen > 0) {
		...
		updateInnerHTML('newCardsCost2', _rectsToBuyLen);
...
function processPreviousCardsToken(_token, key){
	...
	if(_token.token_owner.toUpperCase() != userWalletAddress.toUpperCase()){
		...
		if(usedToOwn){
			...
			if(isBETA)
				$('#lastestCardImg'  + '_previous_' + key).css("background-image", "url(https://api.planetcrypto.app:8443/tronImgShasta/" + key + ")"); 
			else
				$('#lastestCardImg' + '_previous_' + key).css("background-image", "url(https://api.planetcrypto.app:8443/tronImg/" + key + ")"); 
			...
			$('#latestCardHeader' + '_previous_' + key).css("background-color", tokenColor(_token));
			
			updateInnerHTML('latestCardsName' + '_previous_' + key, _token.name);
			updateInnerHTML('latestCardsCardOwner' + '_previous_' + key, window.tronWeb.address.fromHex(_token.token_owner).substring(0,15) + "...");
...
*/
			...
			updateInnerHTML('latestCardsCost' + '_previous_' + key, window.tronWeb.fromSun(_token.current_value.toString()).substring(0,8) + " TRX");
...
*/
		...
		if(validToken){
			...
			$('#userCardHeader' + key).css("background-color", tokenColor(_token));

			if(isBETA)
				$('#lastestCardImg' + key).css("background-image", "url(https://api.planetcrypto.app:8443/tronImgShasta/" + key + ")"); 
			else
				$('#lastestCardImg' + key).css("background-image", "url(https://api.planetcrypto.app:8443/tronImg/" + key + ")"); 
			...
			updateInnerHTML('userCardsCardOwner' + key, vanityOwner(_token, true));
...
function updateUserCards() {
	...
	var latestStart = tokensLen - maxLatest;

	var _boostCardTicker = 0;
...
//		console.log(tokens[key]);
		...
		// have we added this marker???
		if(tokens[key]._markerAdded == true) {
		} else {
			var latLngPoint = L.latLng([parseFloat(hardFixed(tokens[key].plots_lat[0]/1000000,5)), parseFloat(hardFixed(tokens[key].plots_lng[0]/1000000,5))]);
			var _marker = createMarker(key, tokens[key], latLngPoint);
			_markersToAdd.push(_marker);
			markersAllLvls[mymap.getZoom() + ':' + key] = _marker;
		}
		...
		if(foundId == -1){ // new player
			playerStats.push({
				...
				ownerVanity: _token.ownerVanity
			});
		} else {
		...
		if(index == latestStart ) {
			...
			$('#latestCardHeader' + key).css("background-color", tokenColor(_token));

			updateInnerHTML('latestCardsName' + key, _token.name);
			updateInnerHTML('latestCardsCardOwner' + key, vanityOwner(_token,true));
			...
			updateInnerHTML('latestCardsCost' + key, window.tronWeb.fromSun(_token.current_value.toString()).substring(0,8) + " TRX");
			...
			_boostCardTicker++;
			//if(_boostCardTicker > Math.floor(Math.random() * 6) + 3 ) {
			if(_boostCardTicker > 3 ) {
				// add a boost card every 3 new normal cards...
				if(boostCardsShown < boostCards.length) {
					// add in a new one
					/*
        			pack_id: 	results.pack_id,
        			purchase_time: results.purchase_time,
        			...
        			owner: results.owner
        			*/
        			let _boostKey = boostCards[boostCardsShown].pack_id.toString() + ":" + boostCards[boostCardsShown].purchase_time.toString() + ":" + boostCards[boostCardsShown].owner.toString();


        			let _card = "";
					if(boostCards[boostCardsShown].pack_id == 0){
						_card = boostCard0;
					}
					if(boostCards[boostCardsShown].pack_id == 1){
						_card = boostCard1;
					}
					if(boostCards[boostCardsShown].pack_id == 2){
						_card = boostCard2;
					}
					if(boostCards[boostCardsShown].pack_id == 3){
						_card = boostCard3;
					}
					if(boostCards[boostCardsShown].pack_id == 4){
						_card = boostCard4;
					}
					...
					updateInnerHTML('boostScore' + _boostKey, '+' + boostCards[boostCardsShown].empire_score.toString() + ' EMPIRE SCORE');
					updateInnerHTML('bonusCost' + _boostKey, window.tronWeb.fromSun(boostCards[boostCardsShown].price.toString()).substring(0,8) + " TRX");
...
return;
	...
	for(var sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		...
		if(isMobile){
			_row += '<span class="small badge alert">' + (sortedIdx+1) + '</span>';

		} else {
			if(sortedIdx==0)
				_row += '<span class="large badge info" style="width: 40px; height: 40px; font-size: 1.8em;"><i class="fi-trophy" style="color: gold; font-weight: bold;"></i></span>';
			else
				_row += '<center><span class="small badge warning">' + (sortedIdx+1) + '</span></center>';

		}

		if(_sortedPlayers[sortedIdx].ownerVanity.length > 0) {
			_row += '</td><td width="200"><a href="./tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + _sortedPlayers[sortedIdx].ownerVanity + '</a></td>';
		} else {
			if(isMobile)
				_row += '</td><td width="100"><a href="./tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress).substring(0,10) + '...</a></td>';
			else
				_row += '</td><td width="200"><a href="./tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '</a></td>';			
		}
		...
		_row += '<td width="100" style="text-align:center;">' + _sortedPlayers[sortedIdx].totalEmpireScore + '</td>'; //</tr>'

		let _foundDaily = false;
		//console.log(currentDayEmpireScores);
		//console.log("LEN:", currentDayEmpireScores.length);

		let _rowDailyEmpireScore = 0;
		for(let i=0; i< currentDayEmpireScores.length;i++) {
			//console.log(currentDayEmpireScores[i].player.replace("0x", "41").toUpperCase(), _sortedPlayers[sortedIdx].playerAddress.toUpperCase());
			if(currentDayEmpireScores[i].player.replace("0x", "41").toUpperCase() == _sortedPlayers[sortedIdx].playerAddress.toUpperCase()){
				_row += '<td width="100" style="text-align:center;">' + currentDayEmpireScores[i].empireScore + '</td>';
				_rowDailyEmpireScore = currentDayEmpireScores[i].empireScore;
				_foundDaily = true;
				break;
			}
		}
		if(!_foundDaily) {
			_row += '<td width="100" style="text-align:center;">0</td>';
		}
	
		_row += '<td width="100" style="text-align:center;">' + _displayLandSize + _displayLandSizeUnit + ' sq.</td>';

		// % of tax...
		_row += '<td width="100" style="text-align:center;">' + ((parseFloat(_sortedPlayers[sortedIdx].totalEmpireScore) / parseFloat(_totalEmpireScore)) * 100).toFixed(2) + '%</td>';
		//console.log("_rowDailyEmpireScore", _rowDailyEmpireScore, currentDayBonus.empireScoreTotals);
		if(_rowDailyEmpireScore == 0)
			_row += '<td width="100" style="text-align:center;">0 TRX</td>';
		else {
			let _bonus = 
					(
						parseFloat(_rowDailyEmpireScore) / (parseFloat(currentDayBonus.empireScoreTotals) ) * 100
					);

			_row += '<td width="100" style="text-align:right;">' + window.tronWeb.fromSun(currentDayBonusAvail.div(100).times(_bonus)).substring(0,6) + ' TRX</td>';
		}

		//_mobileRow += '<td width="100" style="text-align:center;">' + _sortedPlayers[sortedIdx].totalEmpireScore + '</td></tr>'

		$('#LeaderboardTable tbody').append(_row);


		$('#LeaderboardTable2 tbody').append(_row);
...
var currentDayBonus = {
      ...
      startTS: new BigNumber(0)
};
var currentDayEmpireScores = [];
var currentDayBonusAvail = new BigNumber(0);

var currentDayBonus = new BigNumber(0);
var currentDayEmpireTotal = 0;


function updateDayStats(_callback) {
	PlanetCryptoContract.currentDay().call().then(function(results){

		currentDay = parseInt(results.toString());

		PlanetCryptoContract.day_dayBonuses(currentDay).call().then(function(results){

			currentDayBonus.bonus = new BigNumber(results.bonus.toString());
			currentDayBonus.empireScoreTotals = parseInt(results.empireScoreTotals.toString());
			currentDayBonus.startTS = new BigNumber(results.startTS.toString());
//console.log("currentDayBonus.startTS", currentDayBonus.startTS.toString());
			currentDayEmpireScores = [];

			PlanetCryptoContract.day_dayBonusActivePlayers(currentDay).call().then(function(results){
				for(let i=0; i<results.length; i++) {
					
					async function getEmpireScore() {
						PlanetCryptoContract.day_dayBonusEmpireScore(currentDay, results[i]).call().then(function(res){
							currentDayEmpireScores.push({
								player: results[i],
								empireScore: parseInt(res.toString())
							});
						});
					}
				...
				PlanetCryptoContract.dayBonusAvail(currentDay).call().then(function(results){
					currentDayBonusAvail = new BigNumber(results.toString());

					if(isMainGame){
						if(!udpateClocksRunning)
							updateClocks();
				...
				//console.log("currentDayBonus", currentDayBonus);
...
function updateLeaderboard() {
	...
	// needs redoing...
	// so onstartup we do:
	// 1. getAllPlayerObjectLen()
	// 2. loop all_playerObjects to populate a player array to use throughout session
	// then
	// watch for events from bonus+main contract to update extra empire score + main empire score
	// populate leaderboard from this player object?
	...
	Object.keys(tokens).forEach(function(key,index) {
		...
		let foundId = -1;
		for(let lookupC=0; lookupC<playerStats.length;lookupC++){
			if(playerStats[lookupC].playerAddress){
	...
	*/
	...
	$('#leaderboardTableBody').empty(); //find("tr:gt(0)").remove()
	$('#leaderboardTableBody2').empty(); //find("tr:gt(0)").remove()
	...
	//let _sortedPlayers = playerStatsSortedByEmpireScore();
	let _sortedPlayers = sortPlayers();
	
//console.log(_sortedPlayers);

	let _totalEmpireScore = 0;
	for(let sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		_totalEmpireScore += parseInt(_sortedPlayers[sortedIdx].totalEmpireScore);
	}
console.log("RUNNING UPDATE");

	for(let sortedIdx=0; sortedIdx<_sortedPlayers.length; sortedIdx++){
		let _row = '<tr><td width="50">';
		...
		if(_sortedPlayers[sortedIdx].vanity.length > 0) {
			_row += '</td><td width="200"><a href="./tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + _sortedPlayers[sortedIdx].vanity + '</a></td>';
		} else {
			if(isMobile)
				_row += '</td><td width="100"><a href="./tronCards2.htm?ownerAddress=' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress) + '" >' + window.tronWeb.address.fromHex(_sortedPlayers[sortedIdx].playerAddress).substring(0,10) + '...</a></td>';
			else
		...
		let _displayLandSize = parseInt(_sortedPlayers[sortedIdx].totalLand) * PLOT_SIZE;
		let _displayLandSizeUnit = "m";
		if(_displayLandSize > 99999) {
			_displayLandSize = _displayLandSize / 1000;
			_displayLandSizeUnit = "km";
		}
		...
		// TOTAL EMPIRE SCORE
		_row += '<td width="100">' + _sortedPlayers[sortedIdx].totalEmpireScore + '</td>';


		// LAST 24 HR EMPIRE SCORE
		let _foundDaily = false;
		let _rowDailyEmpireScore = 0;
		for(let i=0; i< currentDayEmpireScores.length;i++) {
			...
			//	console.log("CHECKS FOUND:", currentDayEmpireScores[i].player.replace("0x", "41").toUpperCase(), _sortedPlayers[sortedIdx].playerAddress.toUpperCase())
				console.log(currentDayEmpireScores[i].empireScore);
				_row += '<td width="100" style="text-align:center;">' + currentDayEmpireScores[i].empireScore + '</td>';
				_rowDailyEmpireScore = currentDayEmpireScores[i].empireScore;
				_foundDaily = true;
				break;
		...
		if(!_foundDaily) {
			//console.log("CHECKS NOT FOUND:", _sortedPlayers[sortedIdx].playerAddress.toUpperCase())
			_row += '<td width="100" style="text-align:center;">0</td>';
		}
	

		// LAND SIZE
		_row += '<td width="100">' + _displayLandSize + _displayLandSizeUnit + ' sq.</td>';
		...
		//console.log("_rowDailyEmpireScore", _rowDailyEmpireScore, currentDayBonus.empireScoreTotals);
		if(_rowDailyEmpireScore == 0)
			_row += '<td width="100" style="text-align:right;">0 TRX</td>';
		else {
			let _bonus = 
					(
						parseFloat(_rowDailyEmpireScore) / (parseFloat(currentDayBonus.empireScoreTotals) ) * 100
			...
			_row += '<td width="100" style="text-align:right;">' + window.tronWeb.fromSun(currentDayBonusAvail.div(100).times(_bonus)).substring(0,8) + ' TRX</td>';
		}
...
if(isMainGame){
	...
	if(isMobile)
		userCardsFull = '<div style="width: 170px;">';
	else
		userCardsFull = '<div style="width: 180px;">';
}

userCardsFull += '<div class="userCardOuter" id="userCardFull[]" onclick="showExistingUserCard(this);"><div class="userCardsItemFull" >';
//latestCardsFull+= '                      <div class="large badge alert existingCardBadge" id="latestCardsBadge[]">00000</div>';
userCardsFull+= '';
userCardsFull+= '<div class="card userCardItemHeader" id="userCardHeader[]">';
userCardsFull+= '<span class="userCardHeader">TITLE DEED</span>';
userCardsFull+= '<span class="userCardTitle" id="userCardsName[]"></span>';
userCardsFull+= '</div>';

userCardsFull+= '<div class="card-section2" style="margin: 5px; padding:5px; ">';
userCardsFull+= '		<div id="lastestCardImg[]" class="userCardImg">';
userCardsFull+= '         &nbsp;&nbsp;&nbsp;';
userCardsFull+= '     </div>';

userCardsFull+= ' <img class="icon-compass2"/> <span class="userCardOwner" id="userCardsCardOwner[]">0x0</span></strong><br/> '
userCardsFull+= ' <img class="icon-time"/> <span class="userCardLocation" id="userCardsStartPoint[]">0.0000, 0.0000</span></strong><br/>';
userCardsFull+= ' <img class="icon-resize"/> <span class="userCardSize" id="userCardsTotalPlotsSize[]">0m</span></strong>&nbsp;';
userCardsFull+= ' <img class="icon-tron"/> <span class="userCardPrice" id="userCardsCost[]">0.000 TRX</span> </strong></p>';
...
var latestCardsFull = '<div class="userCardOuter" id="latestCardFull[]" onclick="showLatestUserCard(this);"  ><div class="userCardsItemFull" >';
//latestCardsFull+= '                      <div class="large badge alert existingCardBadge" id="latestCardsBadge[]">00000</div>';
latestCardsFull+= '';
latestCardsFull+= '<div class="card userCardItemHeader" id="latestCardHeader[]">';
latestCardsFull+= '<span class="userCardHeader">TITLE DEED</span>';
latestCardsFull+= '<span class="userCardTitle" id="latestCardsName[]"></span>';
latestCardsFull+= '</div>';

latestCardsFull+= '<div class="card-section2" style="margin: 5px; padding:5px; ">';
latestCardsFull+= '		<div id="lastestCardImg[]" class="userCardImg">';
latestCardsFull+= '         &nbsp;&nbsp;&nbsp;';
latestCardsFull+= '     </div>';

latestCardsFull+= ' <img class="icon-compass2"/> <span class="userCardOwner" id="latestCardsCardOwner[]">0x0</span><br/> '
latestCardsFull+= ' <img class="icon-time"/> <span class="userCardLocation" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
latestCardsFull+= ' <img class="icon-resize"/> <span class="userCardSize" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
latestCardsFull+= ' <img class="icon-tron"/> <span class="userCardPrice" id="latestCardsCost[]">0.000 TRX</span> ';
...
var boostCard0 = '<div class="boostCardOuter" id="boostCardFull[]" onclick="showBoostCardFull(this);" "><div class="userCardsItemFull" >';

boostCard0+= '<div class="card userCardItemHeader" id="boostCardHeader[]" style="background: #9966cc;">';
boostCard0+= '<span class="userCardHeader">TITLE DEED</span>';
boostCard0+= '<span class="userCardTitle" id="boostCardsName[]">Amethyst Boost</span>';
boostCard0+= '</div>';

boostCard0+= '<div class="card-section2" style="margin: 5px; padding:5px; font-size: 0.8em; ">';

boostCard0+= '	<div id="boostImg[]" class="boostCardImg0">';
boostCard0+= '         &nbsp;&nbsp;&nbsp;';
boostCard0+= '   </div>';
boostCard0+= ' <center><span class="bonusEmpire" id="boostScore[]">+000 EMPIRE SCORE</span></center><br/> '

boostCard0+= ' <img class="icon-time hide"/> <span class="userCardLocation hide" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
boostCard0+= ' <img class="icon-resize hide"/> <span class="userCardSize hide" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
boostCard0+= ' <span class="bonusCardPrice" id="bonusCost[]">0.000 TRX</span> ';
boostCard0+= '</div>'
boostCard0+= '</div></div>'

var boostCard1 = '<div class="boostCardOuter" id="boostCardFull[]" onclick="showBoostCardFull(this);" "><div class="userCardsItemFull" >';

boostCard1+= '<div class="card userCardItemHeader" id="boostCardHeader[]" style="background: #e0115f;">';
boostCard1+= '<span class="userCardHeader">TITLE DEED</span>';
boostCard1+= '<span class="userCardTitle" id="boostCardsName[]">Ruby Boost</span>';
boostCard1+= '</div>';

boostCard1+= '<div class="card-section2" style="margin: 5px; padding:5px; font-size: 0.8em; ">';

boostCard1+= '	<div id="boostImg[]" class="boostCardImg1">';
boostCard1+= '         &nbsp;&nbsp;&nbsp;';
boostCard1+= '   </div>';
boostCard1+= ' <center><span class="bonusEmpire" id="boostScore[]">+000 EMPIRE SCORE</span></center><br/> '

boostCard1+= ' <img class="icon-time hide"/> <span class="userCardLocation hide" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
boostCard1+= ' <img class="icon-resize hide"/> <span class="userCardSize hide" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
boostCard1+= ' <span class="bonusCardPrice" id="bonusCost[]">0.000 TRX</span> ';
boostCard1+= '</div>'
boostCard1+= '</div></div>'

var boostCard2 = '<div class="boostCardOuter" id="boostCardFull[]" onclick="showBoostCardFull(this);" "><div class="userCardsItemFull" >';
boostCard2+= '<div class="card userCardItemHeader" id="boostCardHeader[]" style="background: #50c878;">';
boostCard2+= '<span class="userCardHeader">TITLE DEED</span>';
boostCard2+= '<span class="userCardTitle" id="boostCardsName[]">Emerald Boost</span>';
boostCard2+= '</div>';
boostCard2+= '<div class="card-section2" style="margin: 5px; padding:5px; font-size: 0.8em; ">';

boostCard2+= '	<div id="boostImg[]" class="boostCardImg2">';
boostCard2+= '         &nbsp;&nbsp;&nbsp;';
boostCard2+= '   </div>';
boostCard2+= ' <center><span class="bonusEmpire" id="boostScore[]">+000 EMPIRE SCORE</span></center><br/> '

boostCard2+= ' <img class="icon-time hide"/> <span class="userCardLocation hide" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
boostCard2+= ' <img class="icon-resize hide"/> <span class="userCardSize hide" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
boostCard2+= ' <span class="bonusCardPrice" id="bonusCost[]">0.000 TRX</span> ';
boostCard2+= '</div>'
boostCard2+= '</div></div>'

var boostCard3 = '<div class="boostCardOuter" id="boostCardFull[]" onclick="showBoostCardFull(this);" "><div class="userCardsItemFull" >';

boostCard3+= '<div class="card userCardItemHeader" id="boostCardHeader[]" style="background: gold;">';
boostCard3+= '<span class="userCardHeader">TITLE DEED</span>';
boostCard3+= '<span class="userCardTitle" id="boostCardsName[]">Gold Boost</span>';
boostCard3+= '</div>';
boostCard3+= '<div class="card-section2" style="margin: 5px; padding:5px; font-size: 0.8em; ">';

boostCard3+= '	<div id="boostImg[]" class="boostCardImg3">';
boostCard3+= '         &nbsp;&nbsp;&nbsp;';
boostCard3+= '   </div>';
boostCard3+= ' <center><span class="bonusEmpire" id="boostScore[]">+000 EMPIRE SCORE</span></center><br/> '

boostCard3+= ' <img class="icon-time hide"/> <span class="userCardLocation hide" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
boostCard3+= ' <img class="icon-resize hide"/> <span class="userCardSize hide" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
boostCard3+= ' <span class="bonusCardPrice" id="bonusCost[]">0.000 TRX</span> ';
boostCard3+= '</div>'
boostCard3+= '</div></div>'

var boostCard4 = '<div class="boostCardOuter" id="boostCardFull[]" onclick="showBoostCardFull(this);" "><div class="userCardsItemFull" >';

boostCard4+= '<div class="card userCardItemHeader" id="boostCardHeader[]" style="background: #B9F2FF;">';
boostCard4+= '<span class="userCardHeader">TITLE DEED</span>';
boostCard4+= '<span class="userCardTitle" id="boostCardsName[]">Diamond Boost</span>';
boostCard4+= '</div>';

boostCard4+= '<div class="card-section2" style="margin: 5px; padding:5px; font-size: 0.8em; ">';

boostCard4+= '	<div id="boostImg[]" class="boostCardImg4">';
boostCard4+= '         &nbsp;&nbsp;&nbsp;';
boostCard4+= '   </div>';
boostCard4+= ' <center><span class="bonusEmpire" id="boostScore[]">+000 EMPIRE SCORE</span></center><br/> '

boostCard4+= ' <img class="icon-time hide"/> <span class="userCardLocation hide" id="latestCardsStartPoint[]">0.0000, 0.0000</span><br/>';
boostCard4+= ' <img class="icon-resize hide"/> <span class="userCardSize hide" id="latestCardsTotalPlotsSize[]">0m</span>&nbsp;';
boostCard4+= ' <span class="bonusCardPrice" id="bonusCost[]">0.000 TRX</span> ';
boostCard4+= '</div>'
boostCard4+= '</div></div>'
...
function getShareImage() {
	return 'https://api.planetcrypto.app:8443/tronImg/' + existingCardTokenId;
}
