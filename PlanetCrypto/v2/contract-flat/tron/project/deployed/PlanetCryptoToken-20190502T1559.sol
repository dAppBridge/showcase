...
contract PlanetCryptoToken is ERC721Full_custom{
    ...
    event bonusDistributed(uint256 amnt, address to, uint256 timestamp);
    ...
    */
    ...
    struct founderPlayer {
        address playerAddress;
        uint256 totalLand;
        uint256 totalEmpireScore;
        uint256 sunOutstanding;
        bool    activated;
        uint256 sunEarned;
    }
    mapping(address => founderPlayer) internal founderPlayers;
    address[] founderPlayersList;
    uint256   public founderPlayersTotalSUNoutstanding;
    uint256   public founderPlayersTotalSUNoutstandingActivated;
    ...
    // settings
    Percent.percent private m_newPlot_devPercent = Percent.percent(48,100);
    Percent.percent private m_newPlot_taxPercent = Percent.percent(40,100);
    Percent.percent private m_newPlot_bonusPercent = Percent.percent(10,100); 
    ...
    //Percent.percent private m_takeoverPlot_devPercent = Percent.percent(10,100); 
    //Percent.percent private m_takeoverPlot_taxPercent = Percent.percent(10,100); 
    //Percent.percent private m_takeoverPlot_ownerPercent = Percent.percent(80,100);
    ...
    Percent.percent private m_newPlot_founderPlayerPercent = Percent.percent(10,100);
    ...
    function addFounderPlayers(
      ...
      for(uint32 c=0; c<playerAddress.length; c++) {
        if(founderPlayers[playerAddress[c]].playerAddress != playerAddress[c]){

          founderPlayers[playerAddress[c]] = founderPlayer(
              playerAddress[c],
              totalLand[c],
              totalEmpireScore[c],
              sunOutstanding[c],
          ...
          founderPlayersList.push(playerAddress[c]);
          founderPlayersTotalSUNoutstanding += sunOutstanding[c];
          if(activated[c] == true)
            founderPlayersTotalSUNoutstandingActivated += sunOutstanding[c];
        }
    ...
    function getFounderPlayer(address _player) public view returns(
        uint256 totalLand,
        uint256 totalEmpireScore,
        uint256 sunOutstanding,
        bool    activated,
      ...
      if(founderPlayers[_player].playerAddress != _player){
        return;
      } else {
        totalLand = founderPlayers[_player].totalLand;
        totalEmpireScore = founderPlayers[_player].totalEmpireScore;
        sunOutstanding = founderPlayers[_player].sunOutstanding;
        activated = founderPlayers[_player].activated;
        sunEarned = founderPlayers[_player].sunEarned;
      }
    }

    function activateFounderPlayer() public {
      require(founderPlayers[msg.sender].playerAddress == msg.sender, "Player not a founder player");
      require(founderPlayers[msg.sender].activated == false, "Player already activated!");

      founderPlayers[msg.sender].activated = true;
      founderPlayersTotalSUNoutstandingActivated = founderPlayersTotalSUNoutstandingActivated.add(founderPlayers[msg.sender].sunOutstanding);

      all_playerObjects[playerAddressToPlayerObjectID[msg.sender]].founderTaxMarkPoint = _foundingPlayersTaxToDivide;
    }
    ...
    function taxEarningsAvailable() public view returns(uint256) {
        ...
        uint _newFounderTax;
        uint _newTax;
        (_newFounderTax, _newTax) = calcNewTaxEarnings();

        if(_newFounderTax > 0){
          _out = _out + _newFounderTax;
        }
    ...
    function founderTaxEarned() public view returns(uint256 _newEarned, uint256 _existingEarned) {

      uint _newTax;
      (_newEarned, _newTax) = calcNewTaxEarnings();
      _existingEarned = founderPlayers[msg.sender].sunEarned;
    }
    ...
    function withdrawTaxEarning() checkCurrentDay() public {
        ...
        if(_newFounderTax > 0){
          taxEarnings = taxEarnings + _newFounderTax;
          playersFundsOwed[msg.sender] = playersFundsOwed[msg.sender].add(_newFounderTax);
          tax_distributed = tax_distributed.add(_newFounderTax);
          founderPlayersTotalSUNoutstandingActivated = founderPlayersTotalSUNoutstandingActivated.sub(_newFounderTax);
          founderPlayersTotalSUNoutstanding = founderPlayersTotalSUNoutstanding.sub(_newFounderTax);
          founderPlayers[msg.sender].sunOutstanding = founderPlayers[msg.sender].sunOutstanding.sub(_newFounderTax);
          founderPlayers[msg.sender].sunEarned = founderPlayers[msg.sender].sunEarned.add(_newFounderTax);
          all_playerObjects[playerAddressToPlayerObjectID[msg.sender]].founderTaxMarkPoint = _foundingPlayersTaxToDivide;
        }
    ...
    function calcNewTaxEarnings() internal view returns (uint _founderPlayerTax, uint _tax) {
      uint _playersPercent;

      if(founderPlayers[msg.sender].activated == true ) {
        _playersPercent = (founderPlayers[msg.sender].sunOutstanding / founderPlayersTotalSUNoutstandingActivated * 10000000);

        _founderPlayerTax = (_foundingPlayersTaxToDivide - all_playerObjects[playerAddressToPlayerObjectID[msg.sender]].founderTaxMarkPoint) * _playersPercent / 10000000;


        if(_founderPlayerTax > founderPlayers[msg.sender].sunOutstanding)
          _founderPlayerTax = founderPlayers[msg.sender].sunOutstanding;
      }
...
*/
    ...
    uint256 public _foundingPlayersTaxToDivide;
    ...
    function calcPlayerDivs(uint256 _value) internal {
        ...
        if(game_started) {
          ...
          if(founderPlayersTotalSUNoutstandingActivated > 0) {
            // split some of this tax earning out to founder players...
            uint _founderTaxSplit = m_newPlot_founderPlayerPercent.mul(_value);
            if(_founderTaxSplit > founderPlayersTotalSUNoutstandingActivated)
              _founderTaxSplit = founderPlayersTotalSUNoutstandingActivated;

            _foundingPlayersTaxToDivide = _foundingPlayersTaxToDivide.add(_founderTaxSplit);

            _value = _value.sub(_founderTaxSplit);

            if(founderPlayers[msg.sender].activated == true ) {
              _playersPercent = (founderPlayers[msg.sender].sunOutstanding / founderPlayersTotalSUNoutstandingActivated * 10000000); /// / 10000000;
              _playerShare = (_foundingPlayersTaxToDivide - all_playerObjects[playerAddressToPlayerObjectID[msg.sender]].founderTaxMarkPoint) * _playersPercent / 10000000;

              playersFundsOwed[msg.sender] = playersFundsOwed[msg.sender].add(_playerShare);
              tax_distributed = tax_distributed.add(_playerShare);
              //_totalDivs = _totalDivs + _playerShare;
              founderPlayersTotalSUNoutstandingActivated = founderPlayersTotalSUNoutstandingActivated.sub(_playerShare);
              founderPlayersTotalSUNoutstanding = founderPlayersTotalSUNoutstanding.sub(_playerShare);
              founderPlayers[msg.sender].sunOutstanding = founderPlayers[msg.sender].sunOutstanding.sub(_playerShare);
              founderPlayers[msg.sender].sunEarned = founderPlayers[msg.sender].sunEarned.add(_playerShare);
          ...
          //emit taxDistributed(_value, all_playerObjects.length-1, now);
          emit action(
              address(0),
              3,
              _value,
