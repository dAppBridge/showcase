...
contract PlanetCryptoToken is ERC721Full_custom{
    ...
    address planetCryptoUtilsAddress = 0xb042e7717f16b07edf947d28d561109557ff3fbd; // ropsten
    ...
    function processPurchase(bytes32 _name, 
        ...
        emit debugInt(tokenIDCount, plotDetails.length-1);
        tokenIDplotdetailsIndexId[tokenIDCount] = plotDetails.length-1;
        
        // this line failing after using burn(2)
    ...
    function setupPlotOwnership(uint256 _token_id, int256[] _plots_lat, int256[] _plots_lng) internal {
       ...
       for(uint256 c=0;c< _plots_lat.length;c++) {
        ...
        tokenIDlatlngLookup_lat[_token_id] = _plots_lat; // To allow burn lookups
        tokenIDlatlngLookup_lng[_token_id] = _plots_lng; // To allow burn lookups
    ...
    function processBurn(address _token_owner, uint256 _val) internal {
        ...
        for(uint256 c=0;c < tokenIDlatlngLookup_lat[_val].length; c++) {
            latlngTokenID_grids[
                    tokenIDlatlngLookup_lat[_val][c]
                ][tokenIDlatlngLookup_lng[_val][c]] = 0;
        }
        delete tokenIDlatlngLookup_lat[_val];
        delete tokenIDlatlngLookup_lng[_val];
